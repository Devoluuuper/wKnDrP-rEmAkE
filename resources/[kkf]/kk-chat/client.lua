...
RegisterNetEvent('chatMessage', function(title, type, text)
    ...
    local type = (type == 'error' and 'red-700') or (type == 'success' and 'green-700') or
        (type == 'warning' and 'yellow-700') or (type == 'black' and 'zinc-800') or '[#D5AC0F]'


    SendNUIMessage({
        type = 'ON_MESSAGE',
        message = {
            template = '<div class="chat-message block w-full border-l-4 border-' ..
                type ..
                ' chatBoxStyle p-2 text-sm shadow mb-1 rounded-md"><strong class="text-' .. type .. '">{0}:</strong><br> {1}</div>',
            args = { title, text }
        }
    })
end)
