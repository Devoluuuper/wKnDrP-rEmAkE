...
--         category = 'Motorcycles',
--         hash = `shinobi`,
--         img = 'https://gtacars.net/images/68673ab84921c49bfc2b5392a32c84a8'
--     },
...
RegisterServerEvent('kk-vehicleshop:server:buyVehicle', function(name)
    ...
    if xPlayer then
        ...
        local finalPrice = selectedCar.price + ESX.Math.Round(ESX.Math.Percent(taxValue, selectedCar.price))
                ...
                MySQL.Async.execute('UPDATE vehicle_stocks SET stock = stock - 1 WHERE vehicle = @vehicle', {
                    ...
                    if affectedRows > 0 then
                        MySQL.Async.execute('INSERT INTO user_vehicles (owner, plate, vehicle, stored,location, ownername, model) VALUES (?, ?, ?, ?,?, ?, ?)', {
                            ...
                            -- TriggerEvent('Society.AddMoney', 'doj', ESX.Math.Round(ESX.Math.Percent(20, finalPrice)))

                            Wait(500)
                            TriggerClientEvent('KKF.UI.ShowNotification', src, 'info', 'Sõiduk toimetatakse korteri garaazi.')
                        end)
                    else
                        vehicle_stocks[name].stock = vehicle_stocks[name].stock + 1
