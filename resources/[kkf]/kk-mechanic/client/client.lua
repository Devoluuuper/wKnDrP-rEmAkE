...
local function saveVehicle(vehicle)
    local myCar = KKF.Game.GetVehicleProperties(vehicle)
...
local function canInteract()
    for k,v in pairs(cfg.locations) do
        if KKF.PlayerData.job.name == k and KKF.PlayerData.job.onDuty then
            return true
        end
    end
...
--     if canInteract() then
--         exports.ox_inventory:openInventory('stash', KKF.PlayerData.job.name)
--     end
-- end)
...
CreateThread(function()
    ...
    for k,v in pairs(cfg.locations) do
        if k ~= 'harmony' and not k == 'eventure' and not k == 'harmony' then
            KKF.CreateBlip('mechanic_' .. k, v.polyZone.coords, 'Mehaanik', 446, nil, 0.7)
        elseif k == 'eventure' then
            KKF.CreateBlip('mechanic_' .. k, v.polyZone.coords, 'Eventure Automotive', 816, nil, 0.7)
        elseif k == 'harmony' then
            KKF.CreateBlip('mechanic_' .. k, v.polyZone.coords, 'Eriliste sõidukite töökoda', 143, nil, 0.7)
        else
            KKF.CreateBlip('mechanic_' .. k, v.polyZone.coords, 'Mehaanik', 446, nil, 0.7)
        end
                ...
                options = {
                    {
                        ...
                        canInteract = function()
                            return (KKF.PlayerData.job.name == k and KKF.PlayerData.job.onDuty)
                        end
                    }
                }
            })
                ...
                options = {
                    {
                        ...
                        distance = 1.5,
                        canInteract = function()
                            return KKF.PlayerData.job.name == k
                        end
                    },
...
exports.ox_target:addGlobalVehicle({
	{
        ...
        canInteract = function(entity)
            return not cache.vehicle and (canInteract() or (KKF.PlayerData.job.name == 'police' and KKF.PlayerData.job.onDuty)) and not isTrailer(entity)
        end,
        onSelect = function(data)
			local cars = KKF.Game.GetVehiclesInArea(GetEntityCoords(data.entity), 15.0)
                ...
                if progress then
                    ...
                    if attachedEntity then
                        if attachedEntity == NetworkGetNetworkIdFromEntity(data.entity) then
                            KKF.Game.RequestNetworkControlOfEntity(data.entity)
