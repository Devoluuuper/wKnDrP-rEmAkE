                ...
                if money >= args.price then
                    MySQL.Async.fetchAll('SELECT id, items FROM mechanic_deliveries WHERE plate = ? AND job = ?', {args.plate, xPlayer.job.name}, function(result)
                        if result[1] then
                            ...
                            exports['kk-scripts']:sendSocietyLog(xPlayer.source, 'SÕIDUKI JUPID', 'Soetas jupi ' .. KKF.GetItemLabel(args.part .. '_' .. args.mod) .. ' $' .. args.price .. ' eest sõidukile ' .. args.plate .. '.')
            ...
            if result[1] then
                ...
                if possible then
                    if xPlayer.canCarryItem(args.item, 1) then
                        ...
                        MySQL.Async.execute('UPDATE mechanic_deliveries SET items = ? WHERE plate = ? AND job = ?', {json.encode(currentData), args.plate, xPlayer.job.name}, function(result)
                            exports['kk-scripts']:sendSocietyLog(xPlayer.source, 'SÕIDUKI JUPID', 'Väljastas sõiduki ' .. args.plate .. ' jupi ' .. KKF.GetItemLabel(args.item) .. '.')
...
KKF.RegisterUsableItem('fixkit', function(source)
    local src = source
    local xPlayer = KKF.GetPlayerFromId(src)
