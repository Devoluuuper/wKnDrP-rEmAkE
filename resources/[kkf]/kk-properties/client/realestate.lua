...
RegisterCommand('propertiesCad', function()
    ...
    if KKF.PlayerData.job.name == 'properties' and KKF.PlayerData.job.onDuty and KKF.PlayerData.job.permissions.jobMenu then
        showTablet()
    end
end) 
...
RegisterNUICallback("upgradeApartment", function(args, cb)
    ...
    if KKF.PlayerData.identifier == tonumber(args.pid) then
...
RegisterNUICallback("sellWarehouse", function(args, cb)
    if KKF.PlayerData.job.name == 'properties' and KKF.PlayerData.job.onDuty then
...
AddEventHandler('kk-properties:job:openStash', function()
    if KKF.PlayerData.job.name == 'properties' and KKF.PlayerData.job.onDuty and KKF.PlayerData.job.permissions.stash and KKF.PlayerData.job.properties.stash then
        ox_inventory:openInventory('stash', KKF.PlayerData.job.name)
    else
...
for k,v in pairs(cfg.locations) do
    ...
    function point:onEnter()
        if KKF.PlayerData.job.name == 'properties' then
            KKF.ShowInteraction(v.text)
        end
    end
    
    function point:onExit()
        if KKF.PlayerData.job.name == 'properties' then
            KKF.HideInteraction(); exports['kk-police']:setIsInv(false)
        end
    end
    ...
    function point:nearby()
        if self.currentDistance < 2.0 and IsControlJustReleased(0, 38) then
            if KKF.PlayerData.job.name == 'properties' then
                if k == 'stash' and KKF.PlayerData.job.onDuty then
                    if KKF.PlayerData.job.permissions.stash and KKF.PlayerData.job.properties.stash then
                        ox_inventory:openInventory('stash', KKF.PlayerData.job.name)
                    else
