...
function point:onEnter()
    KKF.ShowInteraction('Sisene korterisse', 'E')
end
...
RegisterNetEvent('kk-apartments:client:enterOwn', function(first)
    ...
    if not first then SetEntityCoords(cache.ped, data.interior) end; currentApartment = KKF.PlayerData.identifier
...
--     if currentApartment == KKF.PlayerData.identifier then
...
RegisterNetEvent('KKF.Player.Loaded', function(xPlayer)
    KKF.CreateBlip('apartments', vec3(-270.95, -957.86, 31.22), 'Korter', 475, nil, 0.7)
