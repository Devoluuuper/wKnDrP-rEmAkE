function getVehicleProperties(vehicle)
    if DoesEntityExist(vehicle) then
        ...
        return {
            ...
            tyreBurst = tyreBurst,
            plate = KKF.Functions.GetPlate(vehicle),
            plateIndex = GetVehicleNumberPlateTextIndex(vehicle),

            bodyHealth = KKF.Math.Round(GetVehicleBodyHealth(vehicle), 1),
            engineHealth = KKF.Math.Round(GetVehicleEngineHealth(vehicle), 1),
            tankHealth = KKF.Math.Round(GetVehiclePetrolTankHealth(vehicle), 1),

            fuelLevel = GetVehicleFuelLevel(vehicle) or 0,
            dirtLevel = KKF.Math.Round(GetVehicleDirtLevel(vehicle), 1),
            color1 = colorPrimary,
...
function setVehicleProperties(vehicle, props)
    if DoesEntityExist(vehicle) then
        ...
        if props.mileage then
            Entity(vehicle).state:set('mileage', props.mileage, true)
        end
        
        if props.fuelLevel then
            Entity(vehicle).state:set('fuel', props.fuelLevel, true)
        else
            Entity(vehicle).state:set('fuel', 50, true)
        end
