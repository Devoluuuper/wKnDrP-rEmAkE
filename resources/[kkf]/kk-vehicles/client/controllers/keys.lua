...
function hasKey(plate)
    return exports.ox_inventory:Search('count', 'vehicle_key', { plate = KKF.Math.Trim(plate) }) > 0
end
...
local function getClosestKeyCar()
    ...
    if not cache.vehicle then
        for i = 1, #vehicles do
            ...
            local plate = KKF.Functions.GetPlate(vehicle.vehicle)
...
local function vehLoop()
    hasCurrentKey = hasKey(KKF.Functions.GetPlate(cache.vehicle))
...
local function isOwned(vehicle)
    return lib.callback.await('kk-garages:isPlayerOwned', false, KKF.Functions.GetPlate(vehicle))
end

local function boostingCheck(vehicle, inside)
    local returnable = false
    local canSteal = lib.callback.await('kk-tablet:boosting:isAvailable', false, KKF.Functions.GetPlate(vehicle), inside)

    if canSteal then
        TriggerEvent('kk-tablet:boosting:spawnAgro', KKF.Functions.GetPlate(vehicle)); 
        returnable = true
    else
        returnable = false
    end
...
AddEventHandler('ox_inventory:updateInventory', function(changes)
    ...
    if next(tempKeys) then
        local plate = KKF.Functions.GetPlate(cache.vehicle)
