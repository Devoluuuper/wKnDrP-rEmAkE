        ...
        if kPlayer.adminLevel >= 3 then
            if Player(kPlayer.source).state.adminMode then
                ...
                if type == 'vehicle' then
                    ...
                    lib.callback('KKF.RPC.IsModelReal', kPlayer.source, function(response)
                        if response then
    ...
    if kPlayer then
        if kPlayer.adminLevel >= 2 then
            if Player(kPlayer.source).state.adminMode then
                ...
                if type == 'vehicle' then
                    lib.callback('KKF.RPC.GetVehiclesInArea', kPlayer.source, function(response)
                ...
                elseif type == 'ped' then
                    lib.callback('KKF.RPC.GetPedsInArea', kPlayer.source, function(response)
                        for k,v in pairs(response) do
                            DeleteEntity(NetworkGetEntityFromNetworkId(v))
                        end
                ...
                elseif type == 'object' then
                    lib.callback('KKF.RPC.GetObjectsInArea', kPlayer.source, function(response)
                        DeleteEntity(NetworkGetEntityFromNetworkId(v))
...
RegisterNetEvent('kk-admin2:server:openMenu', function(playerId)
    ...
    if kPlayer then
        if kPlayer.isAdmin() then -- sinu custom funktsioon admini kontrolliks
            local playerSources = KKF.GetPlayers() -- see tagastab numbrid
            local returnable = {}
            local selected = nil

            for i = 1, #playerSources do
                local xPlayer = KKF.GetPlayerFromId(playerSources[i])
                if xPlayer then
...
CreateThread(function()
    while true do
        local tempPlayers = {}
        local playerIds = KKF.GetPlayers() -- tagastab ainult ID-d
        ...
        for i = 1, #playerIds do
            ...
            if kPlayer then
                ...
                tempPlayers[#tempPlayers + 1] = {
                    identifier = kPlayer.steamid,
                    source = playerId,
                    name = kPlayer.name, -- KKF.PlayerData.name kui olemas
    ...
    if kPlayer then
        if kPlayer.adminLevel >= 2 then
            if Player(kPlayer.source).state.adminMode then
                ...
                if Target then
                    TriggerClientEvent('KKF.Player.SetHealth', Target.source, 0)
        ...
        if kPlayer.adminLevel >= 3 then
            if Player(kPlayer.source).state.adminMode then
                ...
                if Target then
                    ...
                    if KKF.DoesJobExist(faction, grade) then
...
local function clearOfflineInventory(source, target)
    ...
    if kPlayer then
        if kPlayer.adminLevel >= 3 then
            if Player(kPlayer.source).state.adminMode then
                local Target = KKF.GetPlayerFromIdentifier(target)
...
AddEventHandler('KKF.Player.Unloaded', function(source)
    if adminMessage then
