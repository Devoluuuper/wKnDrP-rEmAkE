...
RegisterNetEvent('kk-admin2:client:adminKeys', function()
    ...
    if cache.vehicle then
        ...
        vehicle = ESX.Functions.GetClosestVehicle()
    end

    if DoesEntityExist(vehicle) then
        exports['kk-vehicles']:addKey(ESX.Functions.GetPlate(vehicle))
        TriggerServerEvent('kk-admin:server:logs', 'Võttis sõiduki ' .. ESX.Functions.GetPlate(vehicle) ..' võtme.')
...
local function tuneMax()
    ...
    if DoesEntityExist(cache.vehicle) and IsEntityAVehicle(cache.vehicle) then
        ...
        TriggerServerEvent('kk-admin:server:logs', 'Tuunis sõiduki ' .. ESX.Functions.GetPlate(cache.vehicle) .. ' maxi!')
    end
end
