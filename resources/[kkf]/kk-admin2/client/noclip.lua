...
local function noClipToggle(pIsEnabled)
...
TriggerEvent('chat:addSuggestion', '/noclip', 'Aktiveeri noclip.')

local noclipActive = false

RegisterNetEvent('kk-admin2:client:noclip', function()
    if not noclipActive then
        noclipActive = true
        TriggerServerEvent('kk-admin2:server:triggerLog', 'Noclip staatus SEES.')
    else
        noclipActive = false
        TriggerServerEvent('kk-admin2:server:triggerLog', 'Noclip staatus VÄLJAS.')
    end
    
    noClipToggle(noclipActive)
end)

RegisterNUICallback('activateNoclip', function(data, cb)
  TriggerEvent('kk-admin2:client:noclip')
end)