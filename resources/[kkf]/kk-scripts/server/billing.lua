RegisterServerEvent('KKF_billing:sendBill')
AddEventHandler('KKF_billing:sendBill', function(playerId, sharedAccountName, label, amount)
    local src = source
    local xPlayer = KKF.GetPlayerFromId(src)

    if xPlayer then
        local xTarget = KKF.GetPlayerFromId(playerId)
        amount = KKF.Math.Round(amount)
...
lib.callback.register('KKF_billing:getBills', function(source)
	local src = source
	local xPlayer = KKF.GetPlayerFromId(src)
