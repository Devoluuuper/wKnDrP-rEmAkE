...
AddEventHandler(
    ...
    function(target, time, jailer)
        local xPlayer = ESX.GetPlayerFromId(jailer)
        local time = ESX.Math.Round(time) * 60
        ...
        if xPlayer then
            if xPlayer.job.name == 'police' and xPlayer.job.onDuty then
                ...
                if xTarget then
                    ...
                    exports['kk-scripts']:sendLog(xPlayer.identifier, 'VANGISTAMINE', 'Vangistas ajaks ' .. ESX.Math.Round(time / 60) .. ' min.', xTarget.identifier)
                end
            end
        end
    end
...
AddEventHandler(
    ...
    function(target, time)
        ...
        local time = ESX.Math.Round(time)
        ...
        if not jailTimes[xPlayer.identifier] then
            ...
            for k,v in pairs(currentItems) do
                if xPlayer.canCarryItem(k, v) then
                    ...
                    TriggerClientEvent('KKF.UI.ShowNotification', xPlayer.source, 'error', 'Ese ' .. ESX.GetItemLabel(k) .. ' ' .. v .. 'tk ei mahu teile tasku!')
                end
            end
        ...
        if xPlayer then
            if xPlayer.isAdmin() then
                ...
                if xTarget then
                    ...
                    exports['kk-scripts']:sendLog(xPlayer.identifier, 'A-TEAM', 'Vangistas ajaks ' .. ESX.Math.Round(time / 60) .. 'min; Põhjus: ' .. reason .. ';', xTarget.identifier)
                    TriggerClientEvent("chatMessage", -1, 'AJAIL', 1, GetPlayerName(xPlayer.source) .. ' vangistas isiku ' .. GetPlayerName(xTarget.source) .. '; Põhjus: ' .. reason ..'; Aeg: '.. ESX.Math.Round(time / 60) .. ' minutit')
					MySQL.update('INSERT INTO ucp_punishments (steamhex, punishment, description, punisher, timeat) VALUES (?, ?, ?, ?, ?)', { xTarget.steamid, 'AJAIL', reason .. '; Aeg: '.. ESX.Math.Round(time / 60) .. ' minutit' or '', GetPlayerName(xPlayer.source), os.date('%Y-%m-%d %X') })
