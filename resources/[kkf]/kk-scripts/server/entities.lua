...
RegisterCommand('transfervehicle', function(source, args)
	...
	mySteam = KKF.GetPlayerFromId(source).identifier
	myID = KKF.GetPlayerFromId(source).identifier
	myName = KKF.GetPlayerFromId(source).name

	targetSteamName = KKF.GetPlayerFromId(args[1]).name
	targetSteam = KKF.GetPlayerFromId(args[1]).identifier
	...
	MySQL.Async.fetchAll(
        ...
        function(result)
            if result[1] ~= nil then
                local playerName = KKF.GetPlayerFromIdentifier(result[1].owner).identifier
				local pName = KKF.GetPlayerFromIdentifier(result[1].owner).name
				CarOwner = playerName
