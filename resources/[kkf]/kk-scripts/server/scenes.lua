...
SendScenes = function(ply)
    TriggerClientEvent("scenes:send", ply, Scenes)
end
...
RegisterNetEvent("scenes:fetch", function()
    local src = source
    TriggerClientEvent("scenes:sendgamename", src, GetConvar("gamename", "gta5"))
    SendScenes(src)
end)

RegisterNetEvent("scenes:add", function(x, y, z, message, color, distance)
