...
RegisterNetEvent("scenes:sendgamename", function(game)
    IsRDR = game == "rdr3"
    ProcessBinds()
end)
...
CreateScene = function()
    ...
    CreateThread(function()
        ...
        local keyboard = lib.inputDialog('Lisa olukord', {'Sõnum', 'Värv {valge, punane, sinine, tsüaansinine, roheline, kollane, lilla, roosa}', 'Distants {1.1 - 10.0}'})

        if keyboard then
            local message = keyboard[1]; local color = keyboard[2]; local distance = keyboard[3]
            ...
            if not color or not Colors[color] then color = "valge" end
            color = Colors[color]
            
            TriggerServerEvent("scenes:add", x, y, z, message, color, distance)
        end
    end)
end
...
ProcessBinds = function()
    if IsRDR then
        ...
        RegisterKeyMapping("+scenecreate", "(scenes): Pane olukord", "keyboard", "END")
        RegisterKeyMapping("+scenehide", "(scenes): Lülita olukorrad sisse/välja", "keyboard", "")
        RegisterKeyMapping("+scenedelete", "(scenes): Kustuta olukord", "keyboard", "")
    end
end
