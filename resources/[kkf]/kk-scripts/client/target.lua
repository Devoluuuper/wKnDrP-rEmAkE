                ...
                onSelect = function(data)
                    lib.callback('kk-scripts:sitDown', false, function(response)
                        if response then
                            ...
                            ESX.ShowInteraction('Tõuse püsti', 'E')
                            ...
                            ESX.HideInteraction(); SetEntityCoords(cache.ped, vec3(x, y, z + 0.5)) ClearPedTasksImmediately(cache.ped);
                        else
        ...
        options = {
            {
                ...
                canInteract = function(entity, distance, coords)
                    return ESX.PlayerData.job.name == 'akuma'
                end
            }
        }
    })
        ...
        options = {
            {
                ...
                distance = 1.5,
                canInteract = function(entity, distance, coords)
                    return ESX.PlayerData.job.name == 'bubblegum' and ESX.PlayerData.job.onDuty
                end
            }
        }
    })
...
RegisterNetEvent('KKF.Player.Loaded', function(xPlayer)
    ESX.PlayerData = xPlayer; loadTargets()
end)
