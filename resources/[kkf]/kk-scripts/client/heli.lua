			...
			if IsHeliHighEnough(heli) then
				...
				if IsControlJustPressed(0, toggle_rappel) then -- Initiate rappel
					...
					local confirmed = lib.alertDialog({
						header = 'Kas sa soovid köiest laskuda',
						centered = true,
						cancel = true
					})
					if confirmed == 'confirm' then
						...
						TriggerEvent('KKF.UI.ShowNotification', 'error', 'See pole hetkel kõige parem koht selle jaoks')
...
function IsPlayerInPolmav()
	...
	return IsVehicleModel(vehicle, GetHashKey("aw139")) or IsVehicleModel(vehicle, GetHashKey("polas350"))
end
...
function GetVehicleInView(cam)
	...
	local rayhandle = CastRayPointToPoint(coords, coords+(forward_vector*200.0), 10, GetVehiclePedIsIn(cache.ped), 0)
...
function RenderVehicleInfo(vehicle)
	...
	SetTextOutline()
	SetTextEntry("STRING")
	AddTextComponentString(" Auto mudel: "..vehname)
	DrawText(0.45, 0.9)
end
