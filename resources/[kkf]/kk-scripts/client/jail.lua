...
RegisterNetEvent('KKF.Player.Loaded', function(xPlayer)
    ESX.CreateBlip('jail', vec3(1635.277, 2501.774, 45.565), 'Vangla', 188, 1, 0.7)
end)
...
AddEventHandler("kk-jail:setJailed",function(time, justJoined)
	...
	if justJoined then
		...
		JailProcess()
		TriggerEvent("chatMessage", "DOC", 2, "Teil on vanglas jäänud " .. ESX.Math.Round(time / 60) .. " kuud!")
	else
		takeMugshot(
			function()
	TriggerServerEvent('kk-jail:server:confiscateItems')
				...
				TriggerEvent("chatMessage", "DOC", 2, "Teid on vangistatud " .. ESX.Math.Round(time / 60) .. " kuuks!")
			end)
	end
end)
    ...
    function(source, args)
        if isJailed then
            TriggerEvent('KKF.UI.ShowNotification',
                "info",
                "Teil on jäänud vanglas umbes " .. ESX.Math.Round(jailTime / 60) .. " kuud."
            )
        else
...
AddEventHandler('kk-jail:openjailmenu', function()
    local elements = {
        ...
        [3] = {
            ...
            description = 'Sul on aega järele: ' .. ESX.Math.Round(jailTime / 60) .. ' kuud'
        }
    }
...
AddEventHandler('kk-AJail:setAJailed', function(time, justJoined)
        ...
        if justJoined then
            ...
            SetEntityCoords(cache.ped, 2703.6525878906,1619.5941162109,41.82303237915)
            AJailProcess()
            TriggerEvent("chatMessage", "AJAIL", 2, "Teil on admin vanglas jäänud " .. ESX.Math.Round(time / 60) .. " Minutit!")
			TriggerEvent("InteractSound_CL:PlayOnOne", "jaildoor", 1.0)
        else
			TriggerServerEvent('kk-jail:server:confiscateItems')
            AJailTime = time
...
RegisterCommand("ajail", function(source, args)
        if isAJailed then
            TriggerEvent('KKF.UI.ShowNotification',
                "info",
                "Teil on jäänud Admin vanglas veel umbes " .. ESX.Math.Round(AJailTime / 60) .. " minutit."
            )
        else
