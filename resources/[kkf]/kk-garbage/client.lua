...
CreateThread(function()
    KKF.CreateBlip('delivery_garbage', cfg.delivery.coords, 'Jäätmejaam', 171, 3, 0.7)
    ...
    lib.zones.box({
        ...
        onEnter = function()
            inDelivery = true
            KKF.ShowInteraction('Jäätmejaam')
        end,
        onExit = function()
            inDelivery = false
            KKF.HideInteraction()
    ...
    exports.ox_target:addEntity({NetworkGetNetworkIdFromEntity(current.vehicle)}, {
        {
            ...
            onSelect = function()
                hasBag = false; KKF.HideInteraction()
    ...
    CreateThread(function()
        while hasBag do
            if IsControlJustReleased(0, 26) then
                hasBag = false; ClearPedTasks(cache.ped); KKF.HideInteraction()
                DetachEntity(current.trashBag, cache.ped); DeleteObject(current.trashBag)
            end
...
RegisterNetEvent('KKF.Player.Loaded', function(xPlayer)
    ...
    KKF.CreateBlip('garbage', cfg.ped, 'Prügila', 318, 3, 0.7)
    registerPeds()
            ...
            onSelect = function(data)
                if isInZone(current.location.id) then
                    lib.callback('kk-garbage:collectTrash', false, function(canCollect)
                        if canCollect then
                            hasBag = true; KKF.ShowInteraction('Viska prügi maha', 'C')
