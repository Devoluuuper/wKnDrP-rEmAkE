...
RegisterNUICallback('setSellingRecieve', function(args, cb)
    if not isSellingDrugs then
        ...
        SendNUIMessage({action = 'sellingStop'})
        if hasclient then
...
RegisterNetEvent('kkf:onPlayerLogout')
AddEventHandler('kkf:onPlayerLogout', function()
    lib.callback('kk-tablet:selling:setRecieve', false, function(response)
    end, false)
    isSellingDrugs = false
    SendNUIMessage({action = 'sellingStop'})
...
function CreateNewTask()
    if canSell() then
        ...
        TriggerClientEvent('KKF.UI.ShowNotification', -1, 'info', 'Leidsite kliendi, asukoht märgiti GPS-ile')
            ...
            onEnter = function()
                ...
                lib.callback('KKF.Entity.SpawnPed', false, function(networkId)
                    if networkId then
                        ...
                        NetworkRequestControlOfEntity(currentClient)
                        ...
                        exports.ox_target:addEntity({networkId}, {
                            {
                                ...
                                label = 'Proovi hinnaturfi',
                                distance = 1.5,
                                onSelect = function(args)
                                    if not cache.vehicle and not actionInProgress then
                                        lib.callback('drugselling:generateOffers', false, function(deals)
                                            if not deals or #deals == 0 then
                                                TriggerClientEvent('KKF.UI.ShowNotification', -1, 'error', 'Tehinguid ei leitud!')
                                                return
                                            end
                                            local options = {}
                                            for _, deal in ipairs(deals) do
                                                ...
                                                if deal.item == 'bulk_sell' then
                                                    line = string.format(
                                                        "%s\n📦 Esemeid kokku: %d tk\n🧾(-50%%) Kokku: $%d",
                                                        deal.breakdown or "Kõik esemed",
                                                        deal.amount,
                                                        deal.discountedPrice or math.floor(deal.price * 0.5)
                                                    )
                                                else
                                                    line = string.format(
                                                        "🧾 %dx × $%d = $%d\n🎒 Sul on %d tk\n💰 Kasum: $%d ($%d tk kohta)",
                                                        deal.amount, deal.unitPrice, deal.price, deal.count, deal.profit, deal.profitPerItem
                                                    )
                                                end
                                                table.insert(options, {
                                                    ...
                                                    icon = 'handshake',
                                                    onSelect = function()
                                                        startDealAnimation(currentClient, networkId, deal)
                                                    end
                                                })
                                            end
                                            lib.registerContext({
                                            ...
                                            lib.showContext('drug_deal_menu')
                                        end)
                                    else
...
function startDealAnimation(targetPed, networkId, deal)
    local cachedPed = targetPed
    actionInProgress = true
    FreezeEntityPosition(cache.ped, true)
    SetPedTalk(targetPed)
    PlayAmbientSpeech1(targetPed, 'GENERIC_HI', 'SPEECH_PARAMS_STANDARD')
    local obj = exports['kk-scripts']:spawnAttachedObject(targetPed, joaat('hei_prop_heist_cash_pile'), 57005, 0.13, 0.02, 0.0, -90.0, 0, 0)
    TriggerClientEvent('kk-needs:client:addNeed', -1, 'stress', 1000)
    local progress = exports['kk-taskbar']:startAction('sellDrugs', 'Töötled tehingut', 5000, 'actor_berating_loop', 'misscarsteal4@actor', { freeze = true, controls = true })
    if progress then
        ...
        TaskPlayAnim(targetPed, 'mp_common', 'givetake1_a', 8.0, 1.0, -1, 49, 0, 0, 0, 0)
        ...
        currentClient = nil
        RemoveBlip(blip)
        lib.callback('drugselling:performDeal', false, function(success, msg)
            if not success then
                TriggerClientEvent('KKF.UI.ShowNotification', -1, 'error', msg or 'Tehing ebaõnnestus!')
            end
        end, deal)
        Wait(3000)
        DeleteObject(obj)
        ...
        PlayAmbientSpeech1(targetPed, 'GENERIC_FUCK_YOU', 'SPEECH_PARAMS_STANDARD')
        SetPedAsNoLongerNeeded(targetPed)
        FreezeEntityPosition(targetPed, false)
        TaskWanderStandard(targetPed, 10, 10)
        FreezeEntityPosition(cache.ped, false)
...
CreateThread(function()
    lib.callback('paff-robberies:getHouses', false, function(response)
        houseLists = response
    end)
end)

RegisterNetEvent('kk-tablet:selling:removeJob')
AddEventHandler('kk-tablet:selling:removeJob', function()
...
RegisterNUICallback('cancelJobSelling', function(args, cb)
    lib.callback('kk-tablet:selling:cancelJob', false, function(response)
        ...
        if response then
            ...
            RemoveBlip(current.blip)
            current.blip = nil
            currentContractId = nil
            TriggerClientEvent('KKF.UI.ShowNotification', -1, 'info', 'Lõpetasite tööotsa!')
            SendNUIMessage({action = 'removeSellingJobInfo'})
        end
    end, args.id or currentContractId)
end)
...
local function createDelivery(contractId)
    ...
    current.coords = randomLocation.xyz
    SetNewWaypoint(current.coords.x, current.coords.y)
    createBlip()
                                ...
                                lib.callback('kk-tablet:selling:canFinish', false, function(canFinish)
                                    if canFinish then
                                        if cache.vehicle then
                                            ...
                                            if progress then
                                                lib.callback('kk-tablet:selling:finishJob', false, function(success, message)
                                                    PlayAmbientSpeech1(current.npc, 'GENERIC_THANKS', 'SPEECH_PARAMS_STANDARD')
                                                    SendNUIMessage({action = 'removeSellingJobInfo'})
                                                    if current.npc then 
                                                        exports.ox_target:removeEntity({NetworkGetNetworkIdFromEntity(current.npc)}, {'giveSelling'})
                                                        if DoesEntityExist(args.entity) then 
                                                            SetEntityAsMissionEntity(args.entity, true, true)
                                                            DeletePed(args.entity)
                                                        end
                                                        DeletePed(current.npc)
                                                        current.npc = nil
                                                        RemoveBlip(current.blip)
...
RegisterNUICallback('acceptJobSelling', function(args, cb)
    lib.callback('kk-tablet:selling:acceptJob', false, function(success, msg)
        cb({ success = success, message = msg })
        if success then
            currentContractId = args.id
...
RegisterNetEvent('kk-tablet:selling:reloadSelling')
AddEventHandler('kk-tablet:selling:reloadSelling', function()
