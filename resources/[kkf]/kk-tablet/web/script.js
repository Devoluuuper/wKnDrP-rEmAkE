$(document).ready(function () {
    ...
    $(document).on("click", "#foodJobCancel", function () {
        if (actionPossible && jobActive) {
            ...
            $.post('https://kk-tablet/cancelJob', JSON.stringify({}), function(cb) {
                loadingBar(false); jobActive = false; selectedJob = 0
        ...
        if (actionPossible && !jobActive) {
            ...
            $.post('https://kk-tablet/acceptJob', JSON.stringify({id: selectedJob}), function(cb) {
                ...
                if (cb) {
                    ...
                    $('#foodJobAccept').hide(); $('#foodJobWaypoint').show(); $('#foodJobCancel').show()
                } else {
                    functions.loadFoodApp()
                }
            })
    ...
    $(document).on("click", "#foodJobQueueToggle", function () {
        if (actionPossible && !jobActive) {
            ...
            $.post('https://kk-tablet/setRecieve', JSON.stringify(), function(cb) {
                loadingBar(false);
    ...
    $(document).on("click", ".selectFood", function () {
        if (actionPossible && !jobActive) {
            loadingBar(true); selectedJob = $(this).data('id') + +1;
            ...
            $.post('https://kk-tablet/loadJob', JSON.stringify({id: selectedJob}), function(cb) {
                ...
                $('#foodInfo').show(); $('#noFoodInfo').hide()
            })   
        }
    });

    $(document).on("click", ".selectBoostingContract", function () {
        if (actionPossible && !boostingContractActive) {
            loadingBar(true); selectedBoostingContract = $(this).data('id') + +1;

            $.post('https://kk-tablet/loadBoostingContract', JSON.stringify({id: selectedBoostingContract}), function(cb) {
                loadingBar(false); JSON.stringify(cb)

                $('#boostingModel').text(cb.vehicleLabel);
                $('#boostingPlate').text(cb.plate);
                $('#boostingEarning').text('Hind: $' + cb.price); 
                $('#boostingTimeRemaining').text(toHoursAndMinutes(cb.expires));
                $('#boostingImage').attr('src', cb.img);

                $('#boostInfo').show(); $('#noBoostInfo').hide(); $('#buyYourselfContract').show(); $('#sellBoostingContract').show(); $('#acceptBoostingContract').show(); $('#boostingTimeLeft').show();
            })   
        }
    });
    ...
    functions.loadFoodApp = function() {
        if (actionPossible) {
            ...
            $.post('https://kk-tablet/loadFood', JSON.stringify({}), function(cb) {
                ...
                $('#contractList').html('')
                $('#foodJobTop').html('')
                $('#foodJobsCompleted').text(cb.done);
                $('#foodJobsEarned').text(cb.earned);
    ...
    $(document).on("click", "#SellingContrJobWaypoint", function () {
        if (actionPossible && jobActive) {
            loadingBar(true);

            $.post('https://kk-tablet/setWaypointSelling', JSON.stringify({}), function(cb) {
                loadingBar(false)
            })
        }
    }); 
    ...
    $(document).on("click", "#SellingContrJobCancel", function () {
        if (actionPossible && jobActive) {
            ...
            $.post('https://kk-tablet/cancelJobSelling', JSON.stringify({}), function(cb) {
                loadingBar(false); jobActive = false; selectedJob = 0
    ...
    $(document).on("click", ".selectSelling", function () {
        if (actionPossible && !jobActive) {
            ...
            $.post('https://kk-tablet/loadJobSelling', JSON.stringify({id: selectedJob}), function(cb) {
                ...
                $('#SellingContrInfo').show(); $('#noSellingContrInfo').hide()
            })   
        }
    });
    ...
    functions.loadSellingApp = function() {
        if (actionPossible) {
            ...
            $.post('https://kk-tablet/loadSellingData', JSON.stringify({}), function(cb) {
                ...
                $('#sellItemsJobsEarned').text(cb.earned || 0);
                ...
                $.each(cb.contracts, function(k, v){
                    if (v != 'chosen') {
                        ...
                        <div data-id="${k}" class="bg-sky-800 rounded shadow text-sm p-0.5 cursor-pointer selectSelling mb-1">
