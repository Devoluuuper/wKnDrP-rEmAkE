...
RegisterNetEvent('kk-oxyrun:client:startJob', function(data, name)
    ...
    if dealerData then
        ...
        ESX.CreateBlip('oxy_dealer_' .. data.dealer, dealerData.xyz, 'Diiler', 51, 47, 0.8)
    end
end)
...
RegisterNetEvent('kk-oxyrun:client:startSelling', function(data, packages)
    ...
    if locationData then
        ESX.RemoveBlip('oxy_dealer_' .. activeDealer)
        activeDealer = nil
        ...
        ESX.CreateBlip('oxy_sell_' .. data.location, locationData.coords.xyz, 'Müügipunkt', 108, 47, 0.8)
...
RegisterNetEvent('kk-oxyrun:client:cleanRun', function()
    if activeDealer then
        ESX.RemoveBlip('oxy_dealer_' .. activeDealer); activeDealer = nil
    end

    if activeLocation then
        ESX.RemoveBlip('oxy_sell_' .. activeLocation); activeLocation = nil
    end
    ...
    if resource == GetCurrentResourceName() then
        ...
        if cfg.sellLocations then
            for i = 1, #cfg.sellLocations do
                ESX.RemoveBlip('oxy_sell_' .. i)
            end
        end

        if cfg.dealerLocations then
            for i = 1, #cfg.dealerLocations do
                ESX.RemoveBlip('oxy_dealer_' .. i)
            end
        end
