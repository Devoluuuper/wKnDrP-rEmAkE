...
RegisterNetEvent('KKF-radio:use', function()
    toggleRadio(not radioMenu)
end)

RegisterNetEvent('KKF-radio:onRadioDrop', function()
        ...
        if rchannel <= Config.MaxFrequency and rchannel ~= 0 then
            if rchannel ~= RadioChannel then
                if Config.RestrictedChannels[rchannel] ~= nil then
                    if Config.RestrictedChannels[rchannel][KKF.PlayerData.job.name] and KKF.PlayerData.job.onDuty then
...
for i=1, Config.MaxFrequency do
    RegisterNetEvent('KKF-radio:client:JoinRadioChannel'.. i, function(channel)
