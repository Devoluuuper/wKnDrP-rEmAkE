                ...
                MySQL.Async.execute('UPDATE user_labs SET data = ? WHERE id = ?', {json.encode(currentData), args.currentLab}, function()
                    for k,v in pairs(currentUsers) do
                        if v == args.currentLab then
                            TriggerClientEvent('kk-druglabs:client:removePlant', KKF.GetPlayerFromIdentifier(k).source, args.currentPlant)
                        end
                    end
                end)
            end
        ...
        MySQL.Async.fetchAll('SELECT owner, data FROM user_labs WHERE id = ?', {args.currentLab}, function(result)
            if result[1] then
                ...
                MySQL.Async.execute('UPDATE user_labs SET data = ? WHERE id = ?', {json.encode(currentData), args.currentLab}, function()
                    for k,v in pairs(currentUsers) do
                        if v == args.currentLab then
                            TriggerClientEvent('kk-druglabs:client:addPlant', KKF.GetPlayerFromIdentifier(k).source, args.currentPlant, currentData.plants[args.currentPlant])
                        end
                    end
                end)
            end
                                ...
                                MySQL.Async.execute('UPDATE user_labs SET data = ? WHERE id = ?', {json.encode(currentData), currentLab}, function()
                                    for k,v in pairs(currentUsers) do
                                        if v == currentLab then
                                            TriggerClientEvent('kk-druglabs:client:unloadPlants', KKF.GetPlayerFromIdentifier(k).source)
        
                                            SetTimeout(750, function()
                                                TriggerClientEvent('kk-druglabs:client:loadPlants', KKF.GetPlayerFromIdentifier(k).source, currentData)
                                            end)
                                        end
                                    end
...
-- KKF.RegisterUsableItem('roller', function(source)
--     local xPlayer = KKF.GetPlayerFromId(source)
...
-- KKF.RegisterUsableItem('drugscales', function(source)
--     local xPlayer = KKF.GetPlayerFromId(source)
