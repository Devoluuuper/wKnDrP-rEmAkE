...
function startMining(index)
    ...
    if progress then
        KKF.ShowInteraction('Katkesta tegevus', 'C')
...
local function startThreads()
    CreateThread(function()
        KKF.CreateBlip('miner', cfg.blip, 'Kaevandus', 587, 5, 0.7)

        for i = 1, #cfg.smithing do
            local currentId = i
            local currentLocation = cfg.smithing[currentId]

            KKF.CreateBlip('smith_' .. i, currentLocation.coords, 'Sulatus', 417, 5, 0.7)
