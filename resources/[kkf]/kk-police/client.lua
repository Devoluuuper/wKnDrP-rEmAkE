...
RegisterNetEvent('kk-police:client:setCallsign', function(callsign)
	if cache.vehicle then
		...
		if #callsign <= 3 and #callsign > 0 and not hasLetters(callsign) then
			...
			TriggerServerEvent('kk-police:server:sendLog', 'KUTSUNG', 'Auto ' .. KKF.Game.GetPlate(cache.vehicle) .. ' uus kutsung ' .. callsign .. '.')
				...
				exports.ox_target:addLocalEntity(entity, {
					{
						...
						canInteract = function()
							return cache.vehicle and (KKF.PlayerData.job.name == 'police' and KKF.PlayerData.job.onDuty)
						end,
						onSelect = function()
	...
	
					{
						...
						onSelect = function()
							if DoesEntityExist(cache.vehicle) then
								...
								if KKF.PlayerData.job.permissions.leaderMenu then
								...
								if input then 
									if input[1] and input[2] then
										...
										end, NetworkGetNetworkIdFromEntity(cache.vehicle), input[1], input[2], KKF.Game.GetVehicleProperties(cache.vehicle))
									else
								...
								if #options > 0 then
									...
									lib.registerMenu({
										...
										lib.registerMenu({
											...
											KKF.Game.RequestNetworkControlOfEntity(cache.vehicle)
											currentVehicle = exports['kk-vehicles']:getVehicleProperties(cache.vehicle)
											TriggerServerEvent('kk-police:server:sendLog', 'KUTSUNG', 'Auto ' .. KKF.Game.GetPlate(cache.vehicle) .. ' numbrite muutmine.')
											lib.showMenu('police_tuning')
										end)
...
Citizen.CreateThread(function()
	for k,v in pairs(cfg.stations) do
		if not v.noBlip then
			KKF.CreateBlip(k .. '_pd', v.position, v.label, 60, 29, 0.7)
		end
	end
end)
				...
				options = {
					{
						...
						distance = 1.5,
						canInteract = function()
							return KKF.PlayerData.job.name == 'police' and KKF.PlayerData.job.onDuty and KKF.PlayerData.job.permissions.leaderMenu
						end,
			...
			options = {
				{
					...
					distance = 1.5,
					canInteract = function()
						return KKF.PlayerData.job.name == 'police'
					end
				},
			}
		})
...
exports.ox_target:addGlobalPlayer({
	{
		...
		canInteract = function(entity)
			...
			return Player(targetId).state.isCuffed ~= 'none' and not timeout and not cache.vehicle and (KKF.PlayerData.job.name == 'police' and KKF.PlayerData.job.onDuty) and Player(targetId).state.radioChannel
		end,
		onSelect = function(data)
...

	{
		...
		canInteract = function(entity)
			...
			return Player(targetId).state.isCuffed == 'none' and not timeout and not cache.vehicle and (KKF.PlayerData.job.name == 'police' and KKF.PlayerData.job.onDuty) 
		end,
		onSelect = function(data)
			timeout = true
...

	{
		...
		canInteract = function(entity)
			...
			return Player(targetId).state.isCuffed == 'hard' and not timeout and not cache.vehicle and (KKF.PlayerData.job.name == 'police' and KKF.PlayerData.job.onDuty)
		end,
		onSelect = function(data)
			timeout = true
...

	{
		...
		canInteract = function(entity)
			...
			return Player(targetId).state.isCuffed == 'soft' and not timeout and not cache.vehicle and (KKF.PlayerData.job.name == 'police' and KKF.PlayerData.job.onDuty)
		end,
		onSelect = function(data)
			timeout = true
...

	{
		...
		canInteract = function(entity)
			...
			return not cache.vehicle and (KKF.PlayerData.job.name == 'police' and KKF.PlayerData.job.onDuty)
		end,
		onSelect = function(data)
...
RegisterNetEvent('kk-police:client:application', function()
	...
	SendNUIMessage({action = 'show_candidation', data = { name = KKF.PlayerData.name }})
end)
...
RegisterNetEvent('kk-police:client:statement', function()
	SetNuiFocus(true, true)
	SendNUIMessage({action = 'show_statement', data = { name = KKF.PlayerData.name }})
end)
