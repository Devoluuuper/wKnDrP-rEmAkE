...
exports.ox_target:addGlobalPlayer({
    {
        ...
        canInteract = function(entity)
			...
			return (Player(targetId).state.isDead or Player(targetId).state.isCuffed ~= 'none') and not cache.vehicle and ((ESX.PlayerData.job.name == 'police' or ESX.PlayerData.job.name == 'ambulance') and ESX.PlayerData.job.onDuty) and not IsEntityAttachedToEntity(entity, cache.ped)
        end,
...

    {
        ...
        canInteract = function(entity)
			...
			return (Player(targetId).state.isDead or Player(targetId).state.isCuffed ~= 'none') and not cache.vehicle and ((ESX.PlayerData.job.name == 'police' or ESX.PlayerData.job.name == 'ambulance') and ESX.PlayerData.job.onDuty) and IsEntityAttachedToEntity(entity, cache.ped)
        end,
        onSelect = function(data)
