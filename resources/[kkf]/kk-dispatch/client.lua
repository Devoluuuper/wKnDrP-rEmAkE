...
RegisterNetEvent('KKF.Player.JobUpdate', function(job)
    ...
    if not ESX.PlayerData.job.onDuty then
        SendNUIMessage({action = 'closeDispatch'})
    end
end) 
...
RegisterCommand('openDispatch', function()
    if ESX.PlayerData.job.type == 'illegal' then return end
    if not LocalPlayer.state['isLoggedIn'] then return end

    if ESX.PlayerData.job.onDuty and not exports['kk-phone']:canInteract() then
        if not menuOpen then
            ESX.ShowInteraction('Sulge dispatch', 'Z')
...
RegisterNUICallback('closeDispatch', function()
    SetNuiFocus(false, false); SetNuiFocusKeepInput(false); menuOpen = false; ESX.HideInteraction()
end)
...
CreateThread(function()
    while true do
        if not warnedshooting and IsPedShooting(cache.ped) and not IsPedCurrentWeaponSilenced(cache.ped) then
            if ESX.PlayerData.job.name == 'police' and ESX.PlayerData.job.onDuty then return end

            local jobName = 'police' -- Here you can put every job what has been registered by ESX.
            local code = '10-71'
