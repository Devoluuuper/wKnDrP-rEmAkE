...
RegisterNetEvent('kk-mdt:client:openMenu', function()
    if (ESX.PlayerData.job.name == 'police' or ESX.PlayerData.job.name == 'ambulance') and ESX.PlayerData.job.onDuty then
        SendNUIMessage({action = 'open', data = {translate = {}, punishments = cfg.punishments, permissions = ESX.PlayerData.job.permissions, name = ESX.PlayerData.name, job = ESX.PlayerData.job.name, department = lib.callback.await('kk-mdt:requestDepartment', false).myDepartment}}); SetNuiFocus(true, true); exports['kk-scripts']:toggleTab(true)
    end
end)
...
RegisterNUICallback('profileImage', function(args, cb)
    --if canJail then
        lib.callback('kk-mdt:profileImage', false, function(response)
            if response then
                TriggerEvent('kk-police:client:openCamera', args.identifier)
            end
