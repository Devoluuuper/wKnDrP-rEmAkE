...
-- Helper function to get player data using ESX
local function getPlayerData(source)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer then
        return {
            citizenid = xPlayer.identifier, -- Changed from citizenid to identifier
...
lib.callback.register('kk-mdt:createNewCase', function(source)
    ...
    local success, insertId = pcall(MySQL.insert.await,
        'INSERT INTO mdt_cases (citizenid, creator, status, date) VALUES (?, ?, ?, ?)',
        {Player.citizenid, Player.citizenid, 'Avatud', os.time()}
    )
...
lib.callback.register('kk-mdt:updateInfo', function(source, caseId, title, description)
    ...
    local success, result = pcall(function()
        local rowsChanged = MySQL.query.await([[
            UPDATE mdt_cases 
            SET title = ?, description = ?, last_edit = ?, last_editor_id = ?
            WHERE id = ?
        ]], {title, description, os.time(), Player.citizenid, caseId})
        return rowsChanged.affectedRows > 0
    end)
...
-- Add person to case
lib.callback.register('kk-mdt:addPersonCase', function(source, caseId, identifier, memberType, role)
    ...
    MySQL.Async.execute('INSERT INTO mdt_case_members (case_id, player_id, firstname, lastname, role) VALUES (?, ?, ?, ?, ?)', 
        {caseId, identifier, firstname, lastname, role or "Unknown"}
    )
...
lib.callback.register('kk-mdt:addDispatchMembers', function(source, caseId)
    ...
    local citizenid = Player.citizenid or ""
...
lib.callback.register('kk-mdt:loadCase', function(source, caseId)
    ...
    local members = MySQL.query.await([[
        SELECT player_id AS identifier, firstname, lastname, role
    ...
    local officers = MySQL.query.await([[
        SELECT officer_id AS identifier, firstname, lastname, callsign, role
    ...
    for i, v in ipairs(officers) do
        ...
        local playerData = MySQL.query.await('SELECT job, job_grade, callsign AS player_callsign FROM users WHERE identifier = ?', {v.identifier})
    ...
    if caseInfo.creator then
        local creatorData = MySQL.query.await('SELECT firstname, lastname FROM users WHERE identifier = ?', {caseInfo.creator})
    ...
    if caseInfo.last_editor_id then
        local editorData = MySQL.query.await('SELECT firstname, lastname FROM users WHERE identifier = ?', {caseInfo.last_editor_id})
...
lib.callback.register('kk-mdt:searchCasePlayer', function(source, value)
    ...
    local query = [[
        SELECT identifier, firstname, lastname, profilepic
        FROM users
    ...
    if result and #result > 0 then
        for _, row in ipairs(result) do
            ...
            table.insert(profiles, {
                ...
                birthdate = "Unknown", -- ESX does not store birthdate by default
...
lib.callback.register('kk-mdt:removeCaseMember', function(source, caseId, identifier)
...
lib.callback.register('kk-mdt:removeCaseOfficer', function(source, caseId, identifier)
    local rowsChanged = MySQL.Sync.execute(
...
lib.callback.register('kk-mdt:requestProtocolData', function(source, caseId, identifier)
...
lib.callback.register('kk-mdt:updateProtocolData', function(source, caseId, identifier, role, punishments, fine, jail, reduction, guilt)
...
lib.callback.register('kk-mdt:requestOfficerData', function(source, data)
    if type(data) == "number" then
        data = { case = data, identifier = tostring(getPlayerData(source).citizenid) }
    ...
    if officerData and #officerData > 0 then
        ...
        local playerData = MySQL.query.await('SELECT job, job_grade FROM users WHERE identifier = ?', {officer.identifier})
...
-- Update officer data
lib.callback.register('kk-mdt:updateOfficerData', function(source, cb, caseId, identifier, role)
...
lib.callback.register('kk-mdt:addConfiscatedItem', function(source, cb, caseId, identifier, item, amount)
    local Player = getPlayerData(source)
    if not Player then
        TriggerClientEvent('KKF.UI.ShowNotification', source, 'Player data not found!', 5000, 'error')
        return cb(false)
    ...
    MySQL.Async.insert('INSERT INTO mdt_case_items (case_id, identifier, name, amount, officer, uuid) VALUES (?, ?, ?, ?, ?, ?)', {
        caseId,
        identifier,
        item,
        amount,
...
lib.callback.register('kk-mdt:removeConfiscatedItem', function(source, cb, caseId, identifier, uuid)
    MySQL.Async.execute('DELETE FROM mdt_case_items WHERE case_id = ? AND identifier = ? AND uuid = ?', {
        caseId,
        identifier,
        uuid
    }, function(rowsChanged)
...
-- Confirm punishment
lib.callback.register('kk-mdt:confirmPunishment', function(source, cb, caseId, identifier, role, punishments, fine, jail, reduction, faultPoints, guilt)
    if type(punishments) ~= "table" then
        if punishments == nil then
            punishments = {}
        else
    ...
    local canJail = exports['kk-mdt']:canJail(source)
    if (tonumber(jail) > 0 and canJail) or (tonumber(jail) == 0) then
        MySQL.Async.execute('UPDATE mdt_cases SET role = ?, punishments = ?, fine = ?, jail = ?, reduction = ?, strike_points = ?, guilt = ? WHERE id = ? AND citizenid = ?', {
            role,
            json.encode(punishments),
            fine,
            jail,
...
lib.callback.register('kk-mdt:addVehicleItem', function(source, caseId, vehicle, plate)
    ...
    local insertId = MySQL.insert.await(
        ...
        { caseId, plate, vehicle, Player.citizenid, generateUUID() }
    )
