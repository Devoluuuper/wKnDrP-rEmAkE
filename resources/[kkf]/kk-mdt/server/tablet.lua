...
-- Check if player has a valid job and is on duty (custom implementation for ESX)
local function hasJobAndDuty(source, allowedJobs)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then return false end
    local job = xPlayer.job
    -- ESX does not have a native onduty system; assume a custom column `onduty` in users table
...
lib.callback.register('kk-mdt:search', function(source, value)
    ...
    local query = "SELECT identifier, firstname, lastname FROM users WHERE firstname LIKE ? OR lastname LIKE ? OR CONCAT(firstname, ' ', lastname) LIKE ?"
    ...
    if result and #result > 0 then
        ...
        for _, row in ipairs(result) do
            if row.firstname and row.lastname then
                profiles[#profiles + 1] = {
                    citizenid = row.identifier, -- Changed from citizenid to identifier
...
lib.callback.register('kk-mdt:searchVehicles', function(source, value)
    ...
    if result and #result > 0 then
        ...
        for _, row in ipairs(result) do
            ...
            -- ESX vehicle names might need to be fetched from a vehicles table
            ...
            vehicles[#vehicles + 1] = {
                ...
                citizenid = row.owner, -- Changed from citizenid to owner (identifier)
...
lib.callback.register('kk-mdt:loadVehicleProfile', function(source, plate)
    ...
    return {
        plate = row.plate or "",
        citizenid = row.owner or "", -- Changed from citizenid to owner
        ownername = playerName,
        vehicle = {
            model = vehicleModel,
            label = vehicleLabel
...
lib.callback.register('kk-mdt:buyBadge', function(source, worker)
    ...
    if xPlayer.getAccount('bank').money >= price then
        ...
        local query = "UPDATE users SET badge = ? WHERE identifier = ?"
        local newBadge = 'NEW'
...
-- Save worker profile
lib.callback.register('kk-mdt:saveWorkerProfile', function(source, citizenid, department, callsign)
    local src = source
    ...
    if not allowed then
        MySQL.update.await([[
            UPDATE users SET department = ?, callsign = ? WHERE identifier = ?
        ]], { department, callsign, citizenid })
...
-- Load worker profile
lib.callback.register('kk-mdt:loadWorkerProfile', function(source, citizenid)
    if not citizenid or citizenid == '' then
        if cfg.debug then
            debugPrint('Error: Invalid or missing citizenid for source ' .. source)
        end
        return false
    end
    local query = [[
        SELECT identifier, firstname, lastname, job, job_grade, department, callsign, certificates
        FROM users
        WHERE identifier = ?
    ]]
    ...
    if not result or #result == 0 then
        TriggerClientEvent('KKF.UI.ShowNotification', source, 'Mängijat identifier-ga ' .. citizenid .. ' ei leitud!', 5000)
        if cfg.debug then
            debugPrint('No player found for identifier: ' .. citizenid)
        end
        return false
    end
    ...
    if player.certificates then
        ...
        if status then
            ...
            TriggerClientEvent('KKF.UI.ShowNotification', source, 'Viga sertifikaatide laadimisel identifier-le ' .. citizenid, 5000)
        end
    end
    ...
    return {
        citizenid = player.identifier, -- Changed from citizenid to identifier
...
lib.callback.register('kk-mdt:toggleCertificate', function(source, worker, cert)
    local citizenid = worker
    local certKey = cert

    local result = MySQL.query.await("SELECT certificates, job, department FROM users WHERE identifier = ?", { citizenid })
    if not result or #result == 0 then
        TriggerClientEvent('KKF.UI.ShowNotification', source, 'Mängijat identifier-ga ' .. citizenid .. ' ei leitud!', 5000)
        return false
    end
    ...
    local success, err = pcall(function()
        MySQL.update.await("UPDATE users SET certificates = ? WHERE identifier = ?", { json.encode(certs), citizenid })
    end)
...
lib.callback.register('kk-mdt:searchStatements', function(source, value)
    ...
    local query = "SELECT id, identifier, name, email, date FROM mdt_statements WHERE text LIKE ?"
    local result = MySQL.query.await(query, {searchValue})
    return result or {}
end)

-- Load statement
lib.callback.register('kk-mdt:loadStatement', function(source, citizenid)
    if not citizenid then return false end
    local query = "SELECT * FROM mdt_statements WHERE identifier = ? ORDER BY date DESC LIMIT 1"
    local result = MySQL.query.await(query, {citizenid})
...
-- Load player profile
lib.callback.register('kk-mdt:loadProfile', function(source, citizenid)
    local playerQuery = [[
        SELECT u.identifier, u.firstname, u.lastname, u.accounts, u.job, u.job_grade, u.department, 
    ...
    local playerResult = MySQL.query.await(playerQuery, {citizenid})
    ...
    local vehicles = MySQL.query.await(vehiclesQuery, {citizenid}) or {}
    ...
    local profile = {
        ...
        birthdate = "Unknown", -- ESX does not store birthdate by default; adjust if custom field exists
        department = {
            label = deptLabel,
            logo = deptLogo
        },
...
lib.callback.register('kk-mdt:updateWantedProfile', function(source, citizenid, value)
    if not citizenid then return false end
    ...
    local query = "UPDATE users SET wanted = ?, wanted_reason = ? WHERE identifier = ?"
    local result = MySQL.query.await(query, {wantedStatus, reason, citizenid})

    return result and result.affectedRows > 0
end)

-- Save notes
lib.callback.register('kk-mdt:saveNotes', function(source, citizenid, notes)
    if not citizenid or not notes then
        debugPrint(string.format("Error: Invalid input - citizenid: %s, notes: %s", tostring(citizenid), tostring(notes)))
        return false
    end

    local checkQuery = "SELECT 1 FROM users WHERE identifier = ?"
    local checkResult = MySQL.query.await(checkQuery, {citizenid})
    if not checkResult or #checkResult == 0 then
        debugPrint("Error: Identifier " .. citizenid .. " not found in users table")
        return false
    end

    local query = "UPDATE users SET notes = ? WHERE identifier = ?"
    local success, result = pcall(function()
        return MySQL.query.await(query, {notes, citizenid})
    end)
    ...
    if not saved then
        debugPrint("No rows updated for identifier: " .. citizenid)
    end
...
-- Get notes
lib.callback.register('kk-mdt:getNotes', function(source, citizenid)
    if not citizenid then
        debugPrint("Error: Invalid citizenid for getNotes")
        return nil
    end

    local query = "SELECT notes FROM users WHERE identifier = ?"
    local result = MySQL.query.await(query, {citizenid})
    if result and #result > 0 then
...
-- Remove license
lib.callback.register('kk-mdt:removeLicense', function(source, citizenid, license)
    local query = "SELECT accounts FROM users WHERE identifier = ?"
    local result = MySQL.query.await(query, {citizenid})
    if result and #result > 0 then
        ...
        local updateQuery = "UPDATE users SET accounts = ? WHERE identifier = ?"
        local updateResult = MySQL.query.await(updateQuery, {json.encode(accounts), citizenid})
...
lib.callback.register('kk-mdt:createDiagnose', function(source, target, description, done, damages, bill, intensive, prescriptions)
    ...
    local query = "INSERT INTO mdt_diagnoses (identifier, description, done, damages, bill, intensive, prescriptions) VALUES (?, ?, ?, ?, ?, ?, ?)"
...
lib.callback.register('kk-mdt:createCase', function(source, target, description, fine, jail, punishments, strikePoints)
    local query = "INSERT INTO mdt_cases (identifier, description, fine, jail, punishments, strike_points) VALUES (?, ?, ?, ?, ?, ?)"
    ...
    if result and result.affectedRows > 0 and jail > 0 then
        ...
        if xPlayer then
            -- Assuming a jail system compatible with ESX (e.g., esx_jail)
            TriggerEvent('esx_jail:sendToJail', xPlayer.source, jail)
        end
    end
    return result and result.affectedRows > 0
end)
...
lib.callback.register('kk-mdt:loadStaffPage', function(source)
    ...
    local placeholders = string.rep("?,", #cfg.jobs):sub(1, -2)
    local query = ([[
        SELECT j.name as id, j.label, j.logo, j.blip_color, COUNT(u.identifier) as members
        FROM jobs j
...
lib.callback.register('kk-mdt:niceBehaviorAccept', function(source, id, desc, value, name)
    local query = "UPDATE users SET jail = jail - ? WHERE identifier = ?"
