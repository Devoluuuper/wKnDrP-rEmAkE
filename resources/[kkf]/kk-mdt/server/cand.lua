...
-- Insert candidate with identifier
lib.callback.register('kk-mdt:insertCandidateWithTime', function(source, fullName, email, motivation)
    ...
    -- Get identifier from ESX
    local xPlayer = ESX.GetPlayerFromId(source)
    local identifier = xPlayer and xPlayer.identifier or nil

    if not identifier then
        print("Error: No identifier for source " .. tostring(source))
        TriggerClientEvent('KKF.UI.ShowNotification', source, 'Identifier puudub!', 5000, 'error')
        return {success = false, message = "Identifier puudub"}
    end

    local query = "INSERT INTO mdt_candidacies (citizenid, name, email, text, date) VALUES (?, ?, ?, ?, UNIX_TIMESTAMP(NOW()))"
    local insertResult = MySQL.query.await(query, {identifier, fullName, email, motivation})

    if not insertResult then
        print("Insert failed for identifier: " .. tostring(identifier))
...
lib.callback.register('kk-mdt:loadCandidation', function(source, id)
    ...
    local query = "SELECT id, citizenid, name, email, text, date FROM mdt_candidacies WHERE id = ?"
...
lib.callback.register('kk-mdt:searchCandidations', function(source, value)
    local query = "SELECT id, name, citizenid, date FROM mdt_candidacies WHERE name LIKE ?"
