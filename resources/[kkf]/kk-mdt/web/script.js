...
$('#vehicleSearch').keyup(function(e) {
    ...
    $(document).on("click", "#profileImage", function () {
        if (actionPossible && currentProfile.citizenid != 0) {
            ...
            $.post('https://kk-mdt/profileImage', JSON.stringify({ pid: currentProfile.citizenid }), function(cb) {
                loadingBar(false);
...
function loadProfile(citizenid, refresh) {
    if (actionPossible && (currentProfile.citizenid != citizenid || refresh)) {
        ...
        $.post('https://kk-mdt/loadProfile', JSON.stringify({ value: citizenid }), function(cb) {
            ...
            $('#profiledateofbirth').html(cb.dateofbirth || "Unknown");
    ...
    $(document).on("click", ".setNiceBehavior", function () {
        if (actionPossible) {
            selectedDoc = $(this).parent().parent().parent().data('pid');
        ...
        $.post('https://kk-mdt/refreshDocPage', JSON.stringify({}), function(cb) {
            ...
            if (cb) {
                $.each(cb, function(k, v) {
                    ...
                    $('#docList').prepend(`
                        <div data-pid="${v.pid}" data-name="${v.name}" class='border bg-zinc-800 shadow rounded border-1 border-zinc-700 p-2 mb-1 h-min'>
...
functions.loadCase = function(id, callback) {
    ...
    $.post('https://kk-mdt/loadCase', JSON.stringify({ id: id }), function(cb) {
        ...
        $.each(cb.officers || [], function(k, v) {
            ...
            console.log(`Officer ${firstname} ${lastname} (pid: ${pid}): grade = ${grade}, callsign = ${callsign}, role = ${role}`);

            $('#policeCaseOfficersTable').append(`
                <div data-id="${pid}" class="bg-zinc-900 hover:bg-zinc-800 px-4 py-5 grid grid-cols-5 gap-4 border-t border-zinc-700">
        ...
        $.each(cb.members || [], function(k, v){
            ...
            $('#policeCaseItemPersons').append(`<option value="${pid}">${firstname} ${lastname}</option>`);

            $('#policeCasePersonsTable').append(`
                <div data-id="${pid}" class="bg-zinc-900 hover:bg-zinc-800 px-4 py-5 grid grid-cols-4 gap-4 border-t border-zinc-700">
                    <dd class="text-sm text-white">${pid}</dd>
                    <dd class="openCaseProfile underline cursor-pointer text-sm text-white">${firstname} ${lastname}</dd>
                    <dd class="text-sm text-white">${role}</dd>
                    <dd class="flex text-sm text-white justify-center items-center gap-4">
        ...
        $.each(cb.confiscatedItems || [], function(k, v){
            ...
            $('#policeCaseConfiscatedItemsTable').append(`
                <div data-id="${v.pid}" data-uuid="${v.uuid}" class="bg-zinc-900 hover:bg-zinc-800 px-4 py-5 grid grid-cols-5 gap-4 border-t border-zinc-700">
...
$('#casePlayerName').keyup(function(e){
                        ...
                        playerList.append(`
                            <div data-id="${v.citizenid}" class="playerSearchObject flex flex-col items-center gap-4 bg-zinc-900 p-4 rounded border border-zinc-700 hover:border-sky-500 transition duration-200">
                                ...
                                <div class="text-white space-y-1">
                                    ...
                                    <p class="text-sm"><strong>Sünnikuupäev:</strong> ${v.dateofbirth}</p>
    ...
    $(document).on("click", ".playerSearchObject", function () {
        if (actionPossible && currentCase != 0) {
            ...
            $.post('https://kk-mdt/addPersonCase', JSON.stringify({ case: currentCase, pid: id, type: currentType, role: currentRole }), function(cb) {
                loadingBar(false);
    ...
    $(document).on("click", ".removeCaseMember", function () {
        if (actionPossible && currentCase != 0) {
            ...
            $.post('https://kk-mdt/removeCaseMember', JSON.stringify({ case: currentCase, pid: pid }), function(cb) {
                loadingBar(false);
    ...
    $(document).on("click", ".removeCaseOfficer", function () {
        if (actionPossible && currentCase != 0) {
            ...
            $.post('https://kk-mdt/removeCaseOfficer', JSON.stringify({ case: currentCase, pid: pid }), function(cb) {
                loadingBar(false);
    ...
    $(document).on("click", ".editCaseMember", function () {
        if (actionPossible && currentCase != 0) {
            ...
            $.post('https://kk-mdt/requestProtocolData', JSON.stringify({ case: currentCase, pid: pid }), function(cb) {
                loadingBar(false);

                if (cb) {
                    punishmentsChosen = cb.punishments;

                    currentCaseMember = pid;
    ...
    $(document).on("click", "#protocolSaveData", function () {
        if (actionPossible && currentCase != 0 && currentCaseMember != 0) {
            ...
            $.post('https://kk-mdt/updateProtocolData', JSON.stringify({ case: currentCase, pid: currentCaseMember, role: $('#protocolRole').val(), jail: materialPunishment.totalJail, fine: materialPunishment.totalFine, reduction: materialPunishment.reduction, punishments: punishmentsChosen, guilt: $('#guiltSelected').val() }), function(cb) {
                loadingBar(false);
...
$(document).on("click", ".editCaseOfficer", function () {
    if (actionPossible && currentCase != 0) {
        ...
        $.post('https://kk-mdt/requestOfficerData', JSON.stringify({ case: currentCase, pid: pid }), function(cb) {
            loadingBar(false);

            if (cb) {
                currentOfficersMember = pid;
...
$(document).on("click", "#officerSaveData", function () {
    if (actionPossible && currentCase != 0 && currentOfficersMember != 0) {
        ...
        $.post('https://kk-mdt/updateOfficerData', JSON.stringify({ case: currentCase, pid: currentOfficersMember, role: $('#officerProfileRole').val() }), function(cb) {
            loadingBar(false);
    ...
    $(document).on("click", "#addConfiscatedItem", function () {
        if (actionPossible && currentCase != 0) {
            ...
            $.post('https://kk-mdt/addConfiscatedItem', JSON.stringify({ case: currentCase, pid: $('#policeCaseItemPersons').val(), item: $('#confiscatedItemName').val(), amount: $('#confiscatedItemAmount').val() }), function(cb) {
                loadingBar(false);
    ...
    $(document).on("click", ".removeConfiscatedItem", function () {
        if (actionPossible && currentCase != 0) {
            ...
            $.post('https://kk-mdt/removeConfiscatedItem', JSON.stringify({ case: currentCase, pid: pid, uuid: uuid }), function(cb) {
                loadingBar(false);
