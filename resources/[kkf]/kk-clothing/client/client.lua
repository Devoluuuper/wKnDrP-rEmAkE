...
RegisterNetEvent('bluum-appearnance:firstTime', function(val)
    firstTime = val
end)

RegisterNetEvent('bluum-appearnance:loadClothes', function(data)
    LoadPed(data)
end)

RegisterNetEvent('KKF.Player.Loaded', function(xPlayer)
	PlayerLoaded = true; Wait(5000)

    local status = GetResourceKvpString('appearance_blips')

	if not status then
		status = 'disabled'; SetResourceKvp('appearance_blips', status)
	end

    if status == 'enabled' then
        for k,v in pairs(Config.Shops) do
            KKF.CreateBlip(v.type .. '_' .. k, v.pos, v.type == 'tattoomenu' and 'Tattoo pood' or v.type == 'barbermenu' and 'Juuksur' or v.type == 'clothesmenu' and 'Riietepood', v.type == 'tattoomenu' and 75 or v.type == 'barbermenu' and 71 or v.type == 'clothesmenu' and 366, nil, 0.7, nil)
        end
    end
end)

RegisterNetEvent('kk-clothes:client:appearanceBlips', function(type)
	local status = GetResourceKvpString('appearance_blips')
    ...
    if status == 'disabled' then
        SetResourceKvp('appearance_blips', 'enabled')

        for k,v in pairs(Config.Shops) do
            KKF.CreateBlip(v.type .. '_' .. k, v.pos, v.type == 'tattoomenu' and 'Tattoo pood' or v.type == 'barbermenu' and 'Juuksur' or v.type == 'clothesmenu' and 'Riietepood', v.type == 'tattoomenu' and 75 or v.type == 'barbermenu' and 71 or v.type == 'clothesmenu' and 366, nil, 0.7, nil)
        end

        TriggerEvent('KKF.UI.ShowNotification', 'info', 'Riietepoe, tattoo poe ja juuksuri blipid seatud nähtavaks.')
    elseif status == 'enabled' then
        SetResourceKvp('appearance_blips', 'disabled')

        for k,v in pairs(Config.Shops) do
            KKF.RemoveBlip(v.type .. '_' .. k)
        end

        TriggerEvent('KKF.UI.ShowNotification', 'info', 'Riietepoe, tattoo poe ja juuksuri blipid seatud nähtamatuks.')
    end
end)
...
RegisterNetEvent("bluum-appearnance:toggleprop")
AddEventHandler("bluum-appearnance:toggleprop", function(type)
    local all = {}
    all.name = type
    ToggleProps(all)
end)
...
function OpenMenu(name, cb)
    menuName = name; if cb then menuId = cb end
    player = PlayerPedId()
    ...
    if isAllowed then
        ...
        TriggerEvent("KKF.UI.ShowNotification", "error", "Teid ei lubata siia!");
    end
end
...
function Save(save)
    if save then
        data = GetCurrentPed()
        if menuName ~= 'tattoomenu' then
            TriggerServerEvent('bluum-appearnance:save', data)
        end

        if menuName == 'tattoomenu' then
            TriggerServerEvent("bluum-appearnance:saveTattoos", currentTats)
        end
    else
        LoadPed(oldPed)
    end
...
RegisterNetEvent("bluum-appearnance:inService")
AddEventHandler("bluum-appearnance:inService", function(service)
    isService = service
end)

RegisterNetEvent("bluum-appearnance:hasEnough")
AddEventHandler("bluum-appearnance:hasEnough", function(menu)
    if menu == "tattoomenu" then
        TriggerServerEvent("bluum-appearnance:retrieve_tats")
...
RegisterNetEvent('kk-clothing:startPlayerCustomization', function(cb, type)
    OpenMenu(type, cb)
end)

exports('SetSkin', SetSkin)

RegisterNetEvent("bluum-appearnance:settattoos")
AddEventHandler("bluum-appearnance:settattoos", function(playerTattoosList)
...
RegisterNetEvent("bluum-appearnance:setpedfeatures")
AddEventHandler("bluum-appearnance:setpedfeatures", function(data)
    player = PlayerPedId()
    if data == false then
...
for k,v in pairs(Config.Shops) do
    ...
    function point:onEnter()
        if self.type == 'clothesmenu' then
            KKF.ShowInteraction('Ava riietepood', 'E')
        elseif self.type == 'barbermenu' then
            KKF.ShowInteraction('Ava juuksur', 'E')
        elseif self.type == 'tattoomenu' then
            KKF.ShowInteraction('Ava tattoo menüü', 'E')
        end
    end
    ...
    function point:nearby()
        if self.currentDistance < 2.0 and IsControlJustReleased(0, 38) then
            if not cache.vehicle then
                ...
                if money >= Config.Prices[self.type] then
                    SetTimeout(2000, function()
                        TriggerEvent('kk-scripts:client:reloadWeapons')
                    end)

                    TriggerEvent('kk-clothing:startPlayerCustomization', function(done)
                        if done and not firstTime then 
                            TriggerServerEvent('bluum-appearnance:removeMoney', Config.Prices[self.type] + KKF.Math.Round(KKF.Math.Percent(exports['kk-taxes']:getTax('primary').value, Config.Prices[self.type])))
                        end

                        TriggerEvent('KKF.Clothing.Reload'); menuName = ''
                        Wait(500)
                        SetTimeout(2000, function()
                            TriggerEvent('kk-scripts:client:reloadWeapons')
                        end)
                        TriggerServerEvent('kk-clothing:reloadTattoos')
                    end, self.type)
                else
                    TriggerEvent('KKF.UI.ShowNotification', 'error', 'Teil on vaja $' .. Config.Prices[self.type] .. ' + VAT ' .. exports['kk-taxes']:getTax('primary').value .. '%)')
                end
            end
        end
    end
