...
RegisterNetEvent('kk-stancer:client:openMenu', function()
    if KKF.PlayerData.job.name == inZone then
        local data = {
...
for i = 1, #cfg.stancerPoints do
    lib.zones.box({
        ...
        onEnter = function()
            if KKF.PlayerData.job.name == cfg.stancerPoints[i].faction and cache.vehicle then
                inZone = cfg.stancerPoints[i].faction; KKF.ShowInteraction('Stancer')
            end
        end,

        onExit = function()
            if KKF.PlayerData.job.name == cfg.stancerPoints[i].faction then
                inZone = false; KKF.HideInteraction()
            end
        end
    })
end
