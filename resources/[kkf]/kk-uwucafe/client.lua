...
local radioChannel = GetHashKey('uwucafe_radio')

CreateThread(function()
    ESX.PlayerData = ESX.GetPlayerData()

    ESX.CreateBlip('uwucafe', cfg.position, 'Uwu Cafe', 480, 48, 0.7)
end)
        ...
        options = {
            {
                ...
                canInteract = function()
                    return v.type == 'plate' or (ESX.PlayerData.job.name == 'uwucafe' and (ESX.PlayerData.job.onDuty or v.type == 'duty'))
                end,
                ...
                onSelect = function()
                    ...
                    elseif v.type == 'plate' then
                        ox_inventory:openInventory('stash', { id = 'uwucafe_tray_' .. k, owner = false })
                    else
                        exports['kk-crafting']:openCraftMenu('uwucafe_' .. v.type)
                    end
                end
            }
        }
