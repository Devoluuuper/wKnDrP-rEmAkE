...
for k,v in pairs(Config.Hospitals) do
				...
				options = {
					{
						...
						canInteract = function()
							return KKF.PlayerData.job.name == 'ambulance'
						end
					},
				}
			})
	...
	-- 					distance = 1.5,
	-- 					canInteract = function()
	-- 						return (KKF.PlayerData.job.name == 'ambulance' and KKF.PlayerData.job.onDuty) and getNearestPlayer()
	-- 					end,
	-- 					onSelect = function()
...
RegisterNetEvent('kk-ambulance:client:playerSleep', function(coords, tableHeading)
	...
	inBed = true
	KKF.ShowInteraction('Tõuse püsti', 'C')
...
exports.ox_target:addGlobalPlayer({
	{
		...
		canInteract = function(entity)
			...
			return not isBusy and not cache.vehicle and (KKF.PlayerData.job.name == 'ambulance' and KKF.PlayerData.job.onDuty) and Player(targetId).state.isDead
		end,
...

	{
		...
		canInteract = function(entity)
			...
			return not isBusy and not cache.vehicle and (KKF.PlayerData.job.name == 'ambulance' and KKF.PlayerData.job.onDuty) and not Player(targetId).state.isDead
		end,
		onSelect = function(data)
			isBusy = true
