	...
	if v.registration then
		for i = 1, #v.registration do
			...
			function point:onEnter()
				registrationPoint = k
				KKF.ShowInteraction('Registratuur')
			end
	...
	lib.callback('kk-ambulance:requestBed', false, function(response)
		if response then
			...
			if Config.Hospitals[registrationPoint].illegal then
				...
				if (ambulanceOnline >= 1 and not Config.Hospitals[registrationPoint].jail) and not (KKF.PlayerData.job.name == 'ambulance' and KKF.PlayerData.job.onDuty) then
					TriggerEvent('KKF.UI.ShowNotification', 'info', 'Teid viidi palatisse! Oodake meediku saabumist.', 8000)

					SetTimeout(1500, function()
						KKF.ShowInteraction('Lahku voodist', 'C')
					end)
