    ...
    for k,v in pairs(cfg.garages) do
        for k2,v2 in pairs(v.spawnPoints) do
            lib.zones.box({
                ...
                onEnter = function()
                    if ESX.PlayerData.job and (v.faction and ESX.PlayerData.job.name == v.faction and ESX.PlayerData.job.onDuty and ESX.PlayerData.job.permissions.garage) or v.faction == nil or not ESX.PlayerData.job then
                        data.functional = 'garage'
                        data.garage = k 
                        data.type = v.type
                        data.slot = k2
        
                        ESX.ShowInteraction('Garaaž')
                    end
                end,

                onExit = function()
                    if ESX.PlayerData.job and (v.faction and ESX.PlayerData.job.name == v.faction and ESX.PlayerData.job.onDuty) or v.faction == nil or not ESX.PlayerData.job then
                        data.functional = nil
                        data.garage = k 
                        data.type = v.type
                        data.slot = nil
        ...
        for k2,v2 in pairs(v.spawnPoints) do
            lib.zones.box({
                ...
                onEnter = function()
                    ...
                    ESX.ShowInteraction('Kindlustus')
                end,
...
local function blipRefresh()
    for k,v in pairs(cfg.garages) do
        ESX.RemoveBlip(k .. '_garage')

        if v.marker then
            if v.faction and (v.faction == ESX.PlayerData.job.name and ESX.PlayerData.job.onDuty) then
                ESX.CreateBlip(k .. '_garage', v.spawnPoints[1].coords, v.type == 'car' and 'Garaaž' or v.type == 'helicopter' and 'Hangaar' or v.type == 'boat' and 'Sadam', v.type == 'car' and 50 or v.type == 'helicopter' and 64 or v.type == 'boat' and 427, nil, 0.5)
    ...
    for k,v in pairs(cfg.impounds) do
        ESX.CreateBlip(k .. '_impound', v.spawnPoints[1].coords, 'Kindlustus', v.type == 'car' and 500 or v.type == 'helicopter' and 64 or v.type == 'boat' and 427, 60, 0.7)
    end
end
    ...
    if nui and nui.plate then
        ...
        if vehData then
            if IsModelInCdimage(vehData.model) then
                ...
                ESX.Game.SetVehicleProperties(previewActive, vehData)
...
local function openGarage(garage, type)
    ...
    if cfg.garages[garage] then
        ...
        if faction and ESX.PlayerData.job.name ~= faction then
            return 
        end
    end
            ...
            if v.type == type and (v.location == garage or garage == 'prison') and not v.police_impound then
                ...
                if v.stored then
                    ...
                    tabs[vehicleLabel][#tabs[vehicleLabel] + 1] = {
                        ...
                        body = (vehData.bodyHealth and ESX.Math.Round(vehData.bodyHealth / 10) .. '%' or 'N/A'),
                        engine = (vehData.engineHealth and ESX.Math.Round(vehData.engineHealth / 10) .. '%' or 'N/A'),
                        fuel = (vehData.fuelLevel and ESX.Math.Round(vehData.fuelLevel) .. 'L' or 'N/A')
                    }
...
local function storeVehicle(garage, type)
    ...
    if not attached then
        ...
        if isVehicleEmptyOwned(cache.vehicle) then
            ...
            -- end, NetworkGetNetworkIdFromEntity(cache.vehicle), ESX.Game.GetVehicleProperties(cache.vehicle), garage, faction)
        end, ESX.Game.GetVehicleProperties(cache.vehicle), garage, faction)
        else
...
local function calculateImpoundPrice(model)
    ...
    local impoundPrice = ESX.Math.Round(vehiclePrice * (exports['kk-taxes']:getTax('impound').value / 100))
    local realPrice = impoundPrice + ESX.Math.Round(ESX.Math.Percent(exports['kk-taxes']:getTax('primary').value, impoundPrice))
