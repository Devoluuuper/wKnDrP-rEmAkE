...
RegisterNetEvent('kk-truckrobbery:client:destroyRobbery', function()
	RemoveBlip(data.taskBlip); ESX.HideInteraction()
	data = {
		owner = 0,
		taskBlip = nil,
		taskLocation = nil,
...
RegisterNetEvent('kk-truckrobbery:client:openDoors', function(type, coords)
	...
	if dist < 30 then
		...
		elseif type == 'drill' then 
			ESX.ShowInteraction('Korja esemeid', 'E')
		end
...
local function doAction(action, coords)
	if data.owner == ESX.PlayerData.identifier then
		if action == 0 then
...
local function loadLoops()
	CreateThread(function()
		while data.owner == ESX.PlayerData.identifier do
			wait = 5
...
function robberyPoint:onEnter()
	ESX.ShowInteraction('Häki rahaauto asukoht', 'E')
end
