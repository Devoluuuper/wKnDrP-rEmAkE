...
AddEventHandler('KKF.Player.Loaded', function(source, xPlayer)
    ...
    MySQL.Async.fetchAll('SELECT * FROM taxes', {}, function(result)
        if result and #result > 0 then
            local taxMessage = "MAKSUMÄÄRAD:" -- ilma ^4 ja ^7
            for _, tax in ipairs(result) do
                taxMessage = taxMessage .. string.format(" %s: %s%%,", tax.label, tonumber(tax.value))
            end
            
            -- Remove the trailing comma
            taxMessage = taxMessage:sub(1, -2)

            -- Send the message kollase värviga RGB
            TriggerClientEvent("chatMessage", source, "Maksud", {255, 205, 0}, taxMessage)
        end
    end)
end)
