...
RegisterNetEvent('KKF:playerLoaded')
AddEventHandler('KKF:playerLoaded', function(playerData)
...
RegisterNetEvent('KKF:playerDropped')
AddEventHandler('KKF:playerDropped', function()
    for i = 1, #locations do
...
-- Job update event
RegisterNetEvent('KKF:setJob')
AddEventHandler('KKF:setJob', function(job)
    for i = 1, #locations do
        if canEditProperty(locations[i]) then
            local blip = AddBlipForCoord(locations[i].zone.coords.x, locations[i].zone.coords.y, locations[i].zone.coords.z)
            SetBlipSprite(blip, 492)
...
CreateThread(function()
    while not KKF.GetPlayerData().identifier do
        Wait(500)
    end
    print('My identifier:', KKF.GetPlayerData().identifier)
    ...
    for i = 1, #locations do
        ...
        locations[i].zoneBox = lib.zones.box({
            ...
            onEnter = function()
                ...
                TriggerServerEvent('KKF:setMetadata', 'currentProperty', currentProperty)
            ...
            onExit = function()
                ...
                currentProperty = nil
                TriggerServerEvent('KKF:setMetadata', 'currentProperty', nil)
