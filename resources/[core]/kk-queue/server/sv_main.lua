...
_Queue.JoinCbs = {}
_Queue.TempPriority = {}
_Queue.JoinDelay = GetGameTimer() + 30000 and 30000 or 0
...
Queue.InitHostName = Queue.InitHostName ~= "Project X" and Queue.InitHostName or false

MySQL.Async.fetchAll('SELECT * FROM user_permissions', {}, function(result)
    for k,v in pairs(result) do
        _Queue.Priority[string_lower(v.identifier)] = 100
    end
end)

function Queue:DebugPrint(msg)
    if Queue.Debug then
        msg = "^3[kk-queue]: ^0" .. tostring(msg) .. "^7"
        print(msg)
    end
end
...
AddEventHandler('kk-queue:server:playerConnect', function(name, setKickReason, deferrals, src)
    ...
    update(Queue:TempSize() and 'Olete järjekorras ' .. pos .. '/' .. Queue:GetSize() .. ' | Aeg järjekorras: 00:00:00 | Reserveeritud kohad: ' .. Queue:TempSize() or 'Olete järjekorras ' .. pos .. '/' .. Queue:GetSize() .. ' | Aeg järjekorras: 00:00:00')
    ...
    while true do
        ...
        local msg = Queue:TempSize() and 'Olete järjekorras ' .. pos .. '/' .. Queue:GetSize() .. ' | Aeg järjekorras: '.. qTime ..' | Reserveeritud kohad: ' .. Queue:TempSize() or 'Olete järjekorras ' .. pos .. '/' .. Queue:GetSize() .. ' | Aeg järjekorras: ' .. qTime
        update(msg, data.deferrals)
    end
end)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if Queue.InitHostName then 
            SetConvar("sv_hostname", Queue.InitHostName .. (qCount > 0 and " | QUEUE: " .. tostring(qCount) or ""))
        else
            Queue.InitHostName = GetConvar("sv_hostname")
        end
    end
...
RegisterServerEvent("kk-queue:server:playerActivated")
AddEventHandler("kk-queue:server:playerActivated", function()
    local src = source
    local ids = Queue:GetIds(src)
...
SetInterval(function()
    MySQL.Async.fetchAll('SELECT * FROM ucp_users', {}, function(result)
        for k,v in pairs(result) do
            if _Queue.Priority[string_lower(v.steamhex)] then
                if v.prioque > _Queue.Priority[string_lower(v.steamhex)] then
                    _Queue.Priority[string_lower(v.steamhex)] = 100
                end
            else
                _Queue.Priority[string_lower(v.steamhex)] = v.prioque
            end
        end
    end)
end, 3 * 60000)

AddEventHandler('KKF.Queue.AddPriority', function(id, power, temp)
    Queue.Exports:AddPriority(id, power, temp)
end)

AddEventHandler('KKF.Queue.RemovePriority', function(id)
