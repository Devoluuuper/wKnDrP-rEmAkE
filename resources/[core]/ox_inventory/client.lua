...
function client.openInventory(inv, data)
	...
	if inv == 'dumpster' and cache.vehicle then
		return TriggerEvent('KKF.UI.ShowNotification', 'error', locale('inventory_right_access'))
	end
	...
	if canOpenInventory() then
		...
		if inv == 'shop' and invOpen == false then
			if cache.vehicle then 
				return TriggerEvent('KKF.UI.ShowNotification', 'error', locale('cannot_perform'))
			end
		...
		if left then
			...
			if invOpen == false then TriggerEvent('KKF.UI.ShowNotification', 'error', locale('inventory_right_access')) end
			if invOpen then client.closeInventory() end
		end 
	else TriggerEvent('KKF.UI.ShowNotification', 'error', locale('inventory_player_access')) end
end
...
lib.callback.register('ox_inventory:usingItem', function(data)
	...
	if item and usingItem then
		...
		local success = (not item.usetime or noAnim or exports['kk-taskbar']:startAction(item.label, item.label or locale('using', data.metadata.label or data.label), item.usetime, false, false, {freeze = false, controls = false, disableCancel = item.disable})) and not PlayerData.dead

		if success then
			if item.notification then 
				TriggerEvent('KKF.UI.ShowNotification', 'info', item.notification)
			end
...
local function canUseItem(isAmmo)
	...
	and not invBusy
	and not exports['kk-taskbar']:canInteract()
...
local function useSlot(slot)
	...
	if canUseItem(data.ammo and true) then
		if data.component and not currentWeapon then 
			return TriggerEvent('KKF.UI.ShowNotification', 'error', locale('weapon_hand_required'))
		end
		...
		if durability and durability <= 100 and consume then
			if durability <= 0 then 
				return TriggerEvent('KKF.UI.ShowNotification', 'error', locale('no_durability', label))
			elseif consume ~= 0 and consume < 1 and durability < consume * 100 then 
				return TriggerEvent('KKF.UI.ShowNotification', 'error', locale('not_enough_durability', label))
			end
		end
		...
		elseif data.weapon then
            ...
            if data.hash ~= GetSelectedPedWeapon(playerPed) then 
                return TriggerEvent('KKF.UI.ShowNotification', 'error', locale('cannot_use', data.label))
            end
			...
			elseif data.component then
				...
				for componentIndex = 1, #weaponComponents do
					if componentType == Items[weaponComponents[componentIndex]].type then 
						return TriggerEvent('KKF.UI.ShowNotification', 'error', locale('component_slot_occupied', componentType))
					end
				end
				...
				for i = 1, #components do
					...
					if DoesWeaponTakeWeaponComponent(currentWeapon.hash, component) then
						if HasPedGotWeaponComponent(playerPed, currentWeapon.hash, component) then
							TriggerEvent('KKF.UI.ShowNotification', 'error', locale('component_has', label))
						else
				...
				TriggerEvent('KKF.UI.ShowNotification', 'error', locale('component_invalid', label))
...
local function useButton(id, slot)
	if PlayerData.loaded and not invBusy and not exports['kk-taskbar']:canInteract() then
		local item = PlayerData.inventory[slot]
		if not item then return end
...
local KKF = exports['kk-core']:getSharedObject()
...
local function registerCommands()
	local function openGlovebox(vehicle)
		...
		if exports['kk-vehicles']:hasKey(plate) or exports['kk-scripts']:hasGroup(shared.police) or lib.callback.await('kk-garages:checkOwner', false, plate) then
...
--[[			if cache.vehicle then
	...
	lib.addKeybind({
		...
		onPressed = function(self)
			...
			if invBusy or not canOpenInventory() then
				return TriggerEvent('KKF.UI.ShowNotification', 'error', locale('inventory_player_access'))
			end
		...
		onPressed = function(self)
			...
			if currentWeapon.ammo then
				if currentWeapon.metadata.durability > 0 then
					...
					TriggerEvent('KKF.UI.ShowNotification', 'error', locale('no_durability', currentWeapon.label))
				end
			end
		end
	})
	...
	lib.addKeybind({
		...
		onPressed = function()
			if EnableWeaponWheel or IsNuiFocused() or exports['kk-taskbar']:canInteract() then return end
        ...
        if item then
            ...
            if count < 0 then
                if shared.framework == 'KKF' then
                    TriggerEvent('KKF:removeInventoryItem', item.name, item.count)
                end
            ...
            elseif count > 0 then
                if shared.framework == 'KKF' then
                    TriggerEvent('KKF:addInventoryItem', item.name, item.count)
                end
...
RegisterNetEvent('ox_inventory:inventoryReturned', function(data)
	...
	TriggerEvent('KKF.UI.ShowNotification', 'info', locale('items_returned'))
	client.closeInventory()
...
RegisterNetEvent('ox_inventory:inventoryConfiscated', function(message)
	if source == '' then return end 
	if message then TriggerEvent('KKF.UI.ShowNotification', 'info', locale('items_confiscated')) end
	if currentWeapon then currentWeapon = Weapon.Disarm(currentWeapon) end
		...
		if point.isClosest and point.currentDistance < 1.2 then
			...
			if IsControlJustReleased(0, 38) then
				lib.callback('ox_inventory:buyLicense', 1000, function(success, message)
					if success ~= nil then
						TriggerEvent('KKF.UI.ShowNotification', success == false and 'error' or 'success', locale(message, locale('license', point.type:gsub("^%l", string.upper))))
					end
				end, point.invId)
			end
				...
				if currentInventory and not currentInventory.ignoreSecurityChecks then
					if currentInventory.type == 'otherplayer' then
						...
						if not id or #(playerCoords - pedCoords) > 1.8 or not (client.hasGroup(shared.police) or canOpenTarget(ped)) then
							client.closeInventory()
							TriggerEvent('KKF.UI.ShowNotification', 'error', locale('inventory_lost_access'))
						else
...
RegisterNUICallback('removeComponent', function(data, cb)
	...
	if data.slot ~= currentWeapon.slot then 
		return TriggerEvent('KKF.UI.ShowNotification', 'error', locale('weapon_hand_wrong'))
	end
...
RegisterNetEvent('kk-inv:client:takeItemAnim', function()
	Utils.PlayAnim(0, 'mp_common', 'givetake1_a', 1.0, 1.0, 2000, 50, 0.0, 0, 0, 0)
end)
...
RegisterNUICallback('swapItems', function(data, cb)
	...
	elseif response then
		if type(response) == 'table' then
			...
			TriggerEvent('KKF.UI.ShowNotification', 'error', locale(response))
		end
	end
end)
...
RegisterNUICallback('buyItem', function(data, cb)
	...
	if message then
		TriggerEvent('KKF.UI.ShowNotification', message.type or 'info', message.description or '')
	end
...
RegisterNUICallback('craftItem', function(data, cb)
	...
	for i = 1, data.count do
		...
		if not success then 
			if response then TriggerEvent('KKF.UI.ShowNotification', 'error', locale(response or 'cannot_perform')) end
			break
		end
	end
