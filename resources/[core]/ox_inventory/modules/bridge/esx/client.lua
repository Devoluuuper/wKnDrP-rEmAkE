...
function client.setPlayerData(key, value)
	...
	KKF.SetPlayerData(key, value)
end

---@diagnostic disable-next-line: duplicate-set-field
function client.setPlayerStatus(values)
	for name, value in pairs(values) do
		if value > 0 then TriggerEvent('KKF_status:add', name, value) else TriggerEvent('KKF_status:remove', name, -value) end
	end
end

RegisterNetEvent('KKF:onPlayerLogout', client.onLogout)
