...
AddEventHandler('KKF:playerDropped', server.playerDropped)
...
SetTimeout(500, function()
	...
	-- if KKF.CreatePickup then
	-- 	error('ox_inventory requires a KKF Legacy v1.6.0 or above, refer to the documentation.')
	-- end

	server.UseItem = KKF.UseItem
	server.GetPlayerFromId = KKF.GetPlayerFromId

	for _, player in pairs(KKF.Players) do
...
function server.buyLicense(inv, license)
	...
	TriggerEvent('KKF_license:addLicense', inv.id, license.name)
...
function server.isPlayerBoss(playerId)
	local xPlayer = exports['kk-core']:getPlayerFromId(source)

	return xPlayer.job.permissions.leaderMenu
end
