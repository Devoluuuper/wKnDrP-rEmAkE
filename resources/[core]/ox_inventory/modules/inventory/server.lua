...
function Inventory.Save(inv)
    ...
    if inv.player then
        return shared.framework ~= 'KKF' and db.savePlayer(inv.owner, data)
...
function Inventory.AddItem(inv, item, count, metadata, slot, cb)
	...
	if toSlotType == 'number' then
		...
		-- 	exports['kk-scripts']:sendLog(inv.owner, 'INVENTUUR', ('"%s" Lisatud %sx %s to "%s"'):format(invokingResource, count, item.name, inv.label))
		-- end
		exports['kk-scripts']:sendLog(inv.owner, 'INVENTUUR', ('"%s" Lisatud %sx %s to "%s"'):format(invokingResource, count, item.name, inv.label))
	...
	elseif toSlotType == 'table' then
		...
		-- end
		exports['kk-scripts']:sendLog(inv.owner, 'INVENTUUR', ('"%s" lisatud %sx %s to "%s"'):format(invokingResource, added, item.name, inv.label))
...
function Inventory.RemoveItem(inv, item, count, metadata, slot, ignoreTotal)
	...
	if count > 0 then
		...
		if removed > 0 then
			...
			-- 	exports['kk-scripts']:sendLog(inv.owner, 'INVENTUUR', ('"%s" eemaldatud %sx %s from "%s"'):format(invokingResource, removed, item.name, inv.label))
			-- end
			exports['kk-scripts']:sendLog(inv.owner, 'INVENTUUR', ('"%s" eemaldatud %sx %s from "%s"'):format(invokingResource, removed, item.name, inv.label))
...
local function dropItem(source, playerInventory, fromData, data)
	...
	-- 	exports['kk-scripts']:sendLog(playerInventory.owner, 'INVENTUUR', ('%sx %s viis eseme invist "%s" invi "%s"'):format(data.count, toData.name, playerInventory.label, dropId))
	-- end
	exports['kk-scripts']:sendLog(playerInventory.owner, 'INVENTUUR', ('%sx %s viis eseme invist "%s" invi "%s"'):format(data.count, toData.name, playerInventory.label, dropId))
			...
			if toData and ((toData.name ~= fromData.name) or not toData.stack or (not table.matches(toData.metadata, fromData.metadata))) then
				...
				if not sameInventory then
					if (toWeight <= toInventory.maxWeight and fromWeight <= fromInventory.maxWeight) then
						...
						-- end
						exports['kk-scripts']:sendLog(playerInventory.owner, 'INVENTUUR', ('%sx %s viis invist "%s" invi "%s" kogus %sx %s'):format(fromData.count, fromData.name, fromInventory.owner and fromInventory.label or fromInventory.id, toInventory.owner and toInventory.label or toInventory.id, toData.count, toData.name))
			...
			elseif toData and toData.name == fromData.name and table.matches(toData.metadata, fromData.metadata) then
				...
				if fromInventory.type == 'container' or sameInventory or totalWeight <= toInventory.maxWeight then
					...
					if not sameInventory then
						...
						-- 	exports['kk-scripts']:sendLog(playerInventory.owner, 'INVENTUUR', ('%sx %s viis invist "%s" invi "%s"'):format(data.count, fromData.name, fromInventory.owner and fromInventory.label or fromInventory.id, toInventory.owner and toInventory.label or toInventory.id))
						-- end
						exports['kk-scripts']:sendLog(playerInventory.owner, 'INVENTUUR',  ('%sx %s viis invist "%s" invi "%s"'):format(data.count, fromData.name, fromInventory.owner and fromInventory.label or fromInventory.id, toInventory.owner and toInventory.label or toInventory.id))
					end
			...
			elseif data.count <= fromData.count then
				...
				if fromInventory.type == 'container' or sameInventory or (toInventory.weight + toData.weight <= toInventory.maxWeight) then
					...
					if not sameInventory then
						...
						-- if server.loglevel > 0 then
						-- 	lib.logger(playerInventory.owner, 'swapSlots', ('%sx %s transferred from "%s" to "%s"'):format(data.count, fromData.name, fromInventory.owner and fromInventory.label or fromInventory.id, toInventory.owner and toInventory.label or toInventory.id))
						-- 	exports['kk-scripts']:sendLog(playerInventory.owner, 'INVENTUUR',  ('%sx %s invist "%s" invi "%s"'):format(data.count, fromData.name, fromInventory.owner and fromInventory.label or fromInventory.id, toInventory.owner and toInventory.label or toInventory.id))
						-- end
						exports['kk-scripts']:sendLog(playerInventory.owner, 'INVENTUUR',  ('%sx %s invist "%s" invi "%s"'):format(data.count, fromData.name, fromInventory.owner and fromInventory.label or fromInventory.id, toInventory.owner and toInventory.label or toInventory.id))
...
local function prepareInventorySave(inv, buffer, time)
    ...
    if inv.player then
        if shared.framework == 'KKF' then return end
		...
		if TriggerEventHooks('swapItems', {
			...
			if Inventory.AddItem(toInventory, item, count, data.metadata, toSlot) then
				if Inventory.RemoveItem(fromInventory, item, count, data.metadata, slot) then
					...
					exports['kk-scripts']:sendLog(playerInventory.owner, 'INVENTUUR',  ('"%s" andis %sx %s isikule "%s"'):format(fromInventory.label, count, data.name, toInventory.label))
