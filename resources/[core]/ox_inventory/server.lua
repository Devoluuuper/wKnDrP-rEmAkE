...
function server.setPlayerInventory(player, data)
	...
	if inv then
		...
		local maxWeight = MySQL.scalar.await('SELECT max_weight FROM ucp_users WHERE identifier = ?', { steamIdentifier })
		local weight = tonumber(maxWeight) or shared.playerweight

		exports.ox_inventory:SetMaxWeight(player.source, weight)
...
local function openInventory(source, invType, data, ignoreSecurityChecks)
	...
	TriggerEvent('itemRemovalNew', source)
		...
		if durability and consume then
			if durability > 100 then
				...
				if ostime > durability then
					...
					return TriggerClientEvent('KKF.UI.ShowNotification', source, 'error', locale('no_durability', label))
				elseif consume ~= 0 and consume < 1 then
					...
					if percentage < consume * 100 then 
						return TriggerClientEvent('KKF.UI.ShowNotification', source, 'error', locale('not_enough_durability', label))
					end
				end
			...
			if data.count > 1 and consume < 1 and consume > 0 and not Inventory.GetEmptySlot(inventory) then
				return TriggerClientEvent('KKF.UI.ShowNotification', source, 'error', locale('cannot_use', label))
			end
		end
		...
		if item and data and data.count > 0 and data.name == item.name then
			...
			elseif consume then
				if data.count >= consume then
					...
					return TriggerClientEvent('KKF.UI.ShowNotification', source, 'error', locale('item_not_enough', item.name))
				end
			elseif not item.weapon and server.UseItem then
                inventory.usingItem = data
				-- This is used to call an external useItem function, i.e. KKF.UseItem / QBCore.Functions.CanUseItem
				...
				-- should resolve issues, i.e. https://github.com/KKF-framework/KKF-legacy/commit/9fc382bbe0f5b96ff102dace73c424a53458c96e
...
RegisterCommand('convertinventory', function(source, args)
	...
	if not convert then
		return warn('Invalid conversion argument. Valid options: KKF, KKFproperty, qb, linden')
	end
