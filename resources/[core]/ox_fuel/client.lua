...
local function startFueling(vehicle, isPump)
	...
	if isFueling then
		...
		while isFueling do
			if isPump then
				local taxedPrice = Config.priceTick + ESX.Math.Round(ESX.Math.Percent(exports['kk-taxes']:getTax('primary').value, Config.priceTick))
				price += taxedPrice
...
local function getPetrolCan(pumpCoord)
	...
	if progress then
		TriggerServerEvent('ox_fuel:fuelCan', Config.petrolCan.price + ESX.Math.Round(ESX.Math.Percent(exports['kk-taxes']:getTax('primary').value, Config.petrolCan.price)))
	end
end
