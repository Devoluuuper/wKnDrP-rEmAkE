local KKF = exports.es_extended:getSharedObject()
...
if KKF.PlayerLoaded then
    setPlayerData(KKF.PlayerData)
end

RegisterNetEvent('KKF.Player.Loaded', function(data)
...
RegisterNetEvent('KKF.Player.DutyChange', function(value)
    if source == '' then return end
    playerGroups.job.onDuty = value
end)

RegisterNetEvent('KKF:setJob2', function(job)
    if source == '' then return end
    playerGroups.job2 = job
end)

RegisterNetEvent('KKF:addInventoryItem', function(name, count)
    playerItems[name] = count
end)

RegisterNetEvent('KKF:removeInventoryItem', function(name, count)
    playerItems[name] = count
end)
