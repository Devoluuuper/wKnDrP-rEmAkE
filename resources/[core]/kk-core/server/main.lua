...
local function CreateExtendedPlayer(playerId, identifier, adminLevel, accounts, inventory, weight, job, loadout, name, coords, licenses, phone, steamid)
	...
	self.addSociety = function(society, rank)
		...
		if ESX.DoesJobExist(society, rank) then
...
AddEventHandler('playerConnecting', function(name, setCallback, deferrals, skr)
    ...
    local identifier, kickReason = ESX.GetPlayerIdentifier(playerId) or false;
...
AddEventHandler('kk-core:server:registerCharacter', function(data)
    local src = source
    local steamId = ESX.GetPlayerIdentifier(src)
...
RegisterNetEvent('KKF.Player.SessionStarted', function(playerId)
	...
	MySQL.Async.fetchAll('SELECT character_slots FROM ucp_users WHERE steamhex = ?', { ESX.GetPlayerIdentifier(src) }, function(ucp)
		
		MySQL.Async.fetchAll('SELECT * FROM users WHERE identifier = ?', { ESX.GetPlayerIdentifier(src) }, function(result)
...
AddEventHandler('KKF.GetPlayerFromId', function(source, cb) cb(ESX.Players[tonumber(source)]) end)
AddEventHandler('KKF.GetPlayerFromIdentifier', function(identifier, cb) cb(ESX.GetPlayerFromIdentifier(identifier)) end)
        ...
        if GetResourceKvpInt('salaryTime_' .. xPlayer.identifier) == 60 then
			...
			if salary > 0 then
				if job == 'unemployed' then -- unemployed
					ESX.UpdateCheck(xPlayer.source, 'add', salary)
				else
					TriggerEvent('Society.GetMoney', xPlayer.job.name, function(money)
						if money >= salary then
							TriggerEvent('Society.RemoveMoney', xPlayer.job.name, salary); ESX.UpdateCheck(xPlayer.source, 'add', salary)
						else
...
RegisterServerEvent('KKF.ExchangeCheck', function()
	...
	if xPlayer then
		...
		if payCheck > 0 then
			ESX.UpdateCheck(source, 'remove', payCheck); xPlayer.addAccountMoney('bank', payCheck)
