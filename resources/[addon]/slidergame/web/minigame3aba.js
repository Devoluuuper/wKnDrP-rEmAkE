...
let validate = (ev) => {
    ...
    if(parseInt(ev.target.dataset.number, 10) === order){
        ...
        if(order > numbers){
            ...
            $('.message').html('Tegid tublit tööd!');
            $('.groups').addClass('hidden');
            $('.splash').removeClass('hidden');
            
            setTimeout(function () {
                $.post('https://slidergame/callback', JSON.stringify({'success': true}));
        ...
        $('.sub_message').addClass('hidden');
        $('.message').html('Harjutamine teeb meistriks.');
        $('.groups').addClass('hidden');
        $('.splash').removeClass('hidden');
        
        setTimeout(function () {
            $.post('https://slidergame/callback', JSON.stringify({'success': false}));
            $(".minigame").css("display", "none");
            location.reload()
        },1500)
    }
