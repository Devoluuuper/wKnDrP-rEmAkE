...
function GetJob(source)
    return KKF.GetPlayerFromId(source)?.job?.name or "unemployed"
end
...
function GetAllEmployees(job)
    ...
    for i = 1, #employees do
        ...
        local player = KKF.GetPlayerFromIdentifier(employee.identifier)
...
Citizen.CreateThreadNow(function()
    local xPlayers = KKF.GetExtendedPlayers and KKF.GetExtendedPlayers() or KKF.GetPlayers()

    if KKF.GetExtendedPlayers then
        ...
        for _, source in pairs(xPlayers) do
            local job = KKF.GetPlayerFromId(source).job.name
...
---@alias KKFJobData { name: string, onDuty?: boolean, grade: number }

---@param source number
---@param job KKFJobData
---@param lastJob? KKFJobData
...
AddEventHandler("KKF:setJob", function(source, job, lastJob)
    OnJobUpdate(source, job, lastJob)
end)

AddEventHandler("KKF:playerLoaded", function(source, xPlayer)
    OnJobUpdate(source, xPlayer.job)
end)

AddEventHandler("KKF:playerLogout", function(source)
    UnloadJob(source)
end)
