...
while not KKF do
    Wait(500)
    debugprint("Commands: Waiting for KKF to load")
end

if not KKF.RegisterCommand then
    infoprint("warning", "KKF.RegisterCommand not found, admin commands not registered. If you wish to use commands, update your KKF. The phone will still work. Do not ask us for help about this.")
    return
end

KKF.RegisterCommand("toggleverified", "admin", function(xPlayer, args, showError)
...
KKF.RegisterCommand("changepassword", "admin", function(xPlayer, args, showError)
...
KKF.RegisterCommand("resetphonesecurity", "admin", function(xPlayer, args, showError)
    local id = args.id
