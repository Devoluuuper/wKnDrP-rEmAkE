if Config.Item.Inventory ~= "mf-inventory" or not Config.Item.Unique or not Config.Item.Require or Config.Framework ~= "KKF" then
    return
end
...
local function GetItemsByName(source, name)
    local inventory = exports["mf-inventory"]:getInventoryItems(KKF.GetPlayerFromId(source).identifier)
    local items = {}
...
function SetPhoneNumber(source, phoneNumber)
    ...
    local qPlayer = KKF.GetPlayerFromId(source)
...
KKF.RegisterUsableItem(Config.Item.Name, function(source,itemName,remove,item)
    if item then
