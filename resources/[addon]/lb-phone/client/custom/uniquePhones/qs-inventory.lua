...
local function GetItemsByName(name)
    local inventory = (KKF and KKF.GetPlayerData().inventory) or (QB and QB.Functions.GetPlayerData().items) or {}
    local items = {}
...
local function UpdatePhoneAfterInventoryUpdate()
    ...
    -- Wait for the inventory to close, as the item list doesn't update otherwise on KKF
...
RegisterNetEvent("KKF:removeInventoryItem", function(item)
    if item == Config.Item.Name then
        UpdatePhoneAfterInventoryUpdate()
    end
end)

RegisterNetEvent("KKF:addInventoryItem", function(item)
    if item == Config.Item.Name then
        UpdatePhoneAfterInventoryUpdate()
    end
end)
