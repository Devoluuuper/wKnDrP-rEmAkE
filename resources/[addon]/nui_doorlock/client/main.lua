...
function Draw3dNUI(coords, text, color)
	...
	if _x ~= last_x or _y ~= last_y or text ~= lasttext or paused then
		isDrawing = true
		if paused then ESX.HideInteraction() else ESX.ShowInteraction(text) end
			...
			if closestDistance < closestV.maxDistance then
				if not closestV.doors then
					...
					elseif not closestV.locked then
						...
						else if isDrawing then ESX.HideInteraction() isDrawing = false end end
					else
						if IsAuthorized(closestV) then
				...
				if closestDistance > closestV.maxDistance and isDrawing then
					ESX.HideInteraction() isDrawing = false
				end
...
function IsAuthorized(doorID)
	...
	if doorID.authorizedJobs then
		for job,rank in pairs(doorID.authorizedJobs) do
			if job == ESX.PlayerData.job.name and rank <= ESX.PlayerData.job.grade and doorID.dutyRequired == true and ESX.PlayerData.job.onDuty or doorID.dutyRequired ~= true and job == ESX.PlayerData.job.name and rank <= ESX.PlayerData.job.grade then
				canOpen = true
			end
		end
	end
