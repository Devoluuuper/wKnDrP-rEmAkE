...
Citizen.CreateThread(function()
	...
	lib.callback('nui_doorlock:getDoorInfo', false, function(doorInfo)
...
function Draw3dNUI(coords, text, color)
	local paused = false
	...
	if _x ~= last_x or _y ~= last_y or text ~= lasttext or paused then
		isDrawing = true
		if paused then KKF.HideInteraction() else KKF.ShowInteraction(text) end
		...
		if (not specificDoor or doorID == specificDoor) then
			if data.doors then
				for k,v in ipairs(data.doors) do
					if #(vector3(playerCoords.x, playerCoords.y, playerCoords.z) - vector3(v.objCoords.x, v.objCoords.y, v.objCoords.z)) < 100 then
			...
			elseif not data.doors then
				if #(vector3(playerCoords.x, playerCoords.y, playerCoords.z) - vector3(data.objCoords.x, data.objCoords.y, data.objCoords.z)) < 100 then
...
TriggerEvent('chat:addSuggestion', '/doorfix', 'Taaslae ukselukk.', {})
		...
		if doorCount then
			...
			for k,v in pairs(Config.DoorList) do
				if v.setText and v.textCoords then
					distance = #(vector3(v.textCoords.x, v.textCoords.y, v.textCoords.z) - vector3(playerCoords.x, playerCoords.y, playerCoords.z))
...
Citizen.CreateThread(function()
	while true do
		...
		if closestV ~= nil and doorCount ~= nil and doorCount ~= 0 and closestDistance and closestV.setText then
			...
			if closestDistance < closestV.maxDistance then
				if not closestV.doors then
					...
					if closestV.locked and doorState ~= 1 then
						if IsAuthorized(closestV) then
							Draw3dNUI(closestV.textCoords, '[E] - Sulgub', nil)
						else
							Draw3dNUI(closestV.textCoords, 'Sulgub', nil)
						end
					elseif not closestV.locked then
						if Config.ShowUnlockedText then
							if IsAuthorized(closestV) then
								Draw3dNUI(closestV.textCoords, '[E] - Avatud', 'green')
							else
								Draw3dNUI(closestV.textCoords, 'Avatud', 'green')
							end 
						else if isDrawing then KKF.HideInteraction() isDrawing = false end end
					else
						if IsAuthorized(closestV) then
							Draw3dNUI(closestV.textCoords, '[E] - Suletud', 'red')
						else
							Draw3dNUI(closestV.textCoords, 'Suletud', 'red')
						end
					end
				else
					local door = {}
					...
					if door[1] == true and door[2] == true then 
						if IsAuthorized(closestV) then
							Draw3dNUI(closestV.textCoords, '[E] - Suletud', 'red')
						else
							Draw3dNUI(closestV.textCoords, 'Suletud', 'red')
						end
					elseif not closestV.locked then if Config.ShowUnlockedText then 
						if IsAuthorized(closestV) then
							Draw3dNUI(closestV.textCoords, '[E] - Avatud', 'green')
						else
							Draw3dNUI(closestV.textCoords, 'Avatud', 'green')
				...
				if closestDistance > closestV.maxDistance and isDrawing then
					KKF.HideInteraction() isDrawing = false
				end
...
function IsAuthorized(doorID)
	local canOpen = false
	if KKF.PlayerData.job == nil then
		return false
	end
	if doorID.authorizedJobs then
		for job,rank in pairs(doorID.authorizedJobs) do
			if job == KKF.PlayerData.job.name and rank <= KKF.PlayerData.job.grade and doorID.dutyRequired == true and KKF.PlayerData.job.onDuty or doorID.dutyRequired ~= true and job == KKF.PlayerData.job.name and rank <= KKF.PlayerData.job.grade then
				canOpen = true
			end
		end
	end
...
RegisterCommand('doorlock', function()
	if not LocalPlayer.state.isDead and not LocalPlayer.state.isCuffed and closestDoor and IsAuthorized(closestV) then
...
RegisterNetEvent('nui_doorlock:client:openNearby', function()
	local locked = false
	TriggerServerEvent('nui_doorlock:updateState', closestDoor, locked, nil, true, true) -- Broadcast new state of the door to everyone
end)

RegisterKeyMapping('doorlock', 'Ukselukk', 'keyboard', 'e')

RegisterNetEvent('KKF_lockpick:onUse') -- Modify for whichever lockpick you're using
AddEventHandler('KKF_lockpick:onUse', function()
	if cache.vehicle then
		TriggerEvent('kk-scripts:client:doLockpick')
	else
		if not isDead and not LocalPlayer.state.isCuffed and closestDoor and closestV.lockpick then
			TaskTurnPedToFaceCoord(playerPed, closestV.textCoords, 0)
	
			TriggerEvent('kk-skillcircle:client:openMinigame', function(skill)
				if skill then
					local locked = not closestV.locked
					...
					if chance > 75 then
						TriggerServerEvent('KKF.Player.RemoveItem', 'lockpick', 1)
					end
		
					TriggerEvent('KKF.UI.ShowNotification', 'error', 'Te ei saanud muukimisega hakkama!')
				end
			end, math.random(2,4), 5)
		end
	end
end)
